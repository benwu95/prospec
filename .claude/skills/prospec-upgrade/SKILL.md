...
description: "Prospec Version Upgrade Finisher - after `prospec upgrade` records the version, syncs agents, and reports gaps, work through the report's docs inventory: update init-created files whose format drifted and create missing ones (asking consent per file), then localize triggers for newly-added skills and re-sync — version upgrades only, not first-time onboarding. Triggers: upgrade prospec, prospec upgrade, migrate prospec version, 升級 prospec, prospec 升級, 遷移 prospec 版本"
---
...
- `Docs inventory:` — one line per init-created doc, `✓ <path> (template: <hbs>)` when present or
  ...
  Step 2's safety net.
- `stale Language Policy wording: …` — the Language Policy rule in `prospec/CONSTITUTION.md` has a
  Description that no longer matches the rule rendered for this project's resolved language scope
  (an untouched old seed, a reworded Description, or a changed `artifact_language` /
  `trust_zone_language`), so it contradicts the entry config (Step 2.5). It is immediately
...
authored-wording change this skill may propose, because that principle's Description is **generated**
from the project's resolved language scope (the same scope the entry config renders), not something the
owner authored. The line fires for three causes — an untouched old seed, a Description reworded away
from the generated one, or an `artifact_language` / `trust_zone_language` changed after the rule was
written — and the remedy is the same for all three.
...
1. Tell the user their project has no `artifact_language`, so change artifacts currently default
   to **English**, and ask which language they want for them (default: English; the trust zone's
   language is governed separately by `trust_zone_language`, which this step never sets).
...
1. Run `prospec agent triggers` (Bash) to get a ready-to-translate scaffold with two blocks —
   `skill_triggers` (invocation words) and `skill_exclusions` (short phrases naming what a skill is NOT
   for) — listing only the skills still missing an entry in each, each with its English baseline sourced
   from the CLI (authoritative; never grep a deployed SKILL.md, whose frontmatter already merges custom
   words); translate both blocks and write both back with `--write`
...
- **NEVER** run Step 2.5 unprompted — it fires only on the report's `stale Language Policy wording` line, so a Description that matches the resolved scope is never touched
