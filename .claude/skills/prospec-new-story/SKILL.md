...
3. **Stated Assumptions**: 100% list all autonomous inferences (derived name, scale selection reasoning, default boundaries, constraint assumptions) in the project's configured `artifact_language`
4. **Edge Cases**: Known boundary conditions and error scenarios
5. **Functional Requirements**: Numbered FR-001, FR-002... for traceability
6. **Success Criteria**: Measurable SC-001, SC-002...
7. **Related Modules**: Cross-reference proposal terms (feature names, domain concepts) against `prospec/index.md` module keywords. List matched modules with relevance reasoning.
8. **Open Questions**: Mark `NEEDS CLARIFICATION` for ambiguities
