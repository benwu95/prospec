...
1. **Implementation**: Execute code tasks sequentially by layer (Types → Lib → Services → CLI → Tests). Invoke dynamic project test command (`check --record-tests` or detected runner) to ensure tests pass. Set `status: implemented`.
