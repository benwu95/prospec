...
5. [DYNAMIC] Read `prospec/ai-knowledge/_index.md` — if exists
...
- **Primary first**: a module appears under its primary (`category[0]`) `### {Category}` heading
  **exactly once** — never list it under two headings (L0 must stay duplicate-free). Secondary
  categories live only in `module-map.yaml` (surfaced by the MCP `search_modules` join), not in `_index.md`.
- Keep the whole L0 within budget regardless of grouping (≤ {l0_max} tokens).
...
- _index.md has Aliases + Rationale columns and Loading Rules section
...
- [ ] _index.md has the module table + Loading Rules
- [ ] L0 total <= 1,500 tokens
- [ ] >= 2 modules
