...
> Prospec is a language-agnostic and architecture-agnostic SDD framework. The Architecture Verifier dynamically reads the downstream project's `prospec/CONSTITUTION.md` and `prospec/ai-knowledge/_conventions.md`. It **never** hardcodes any specific framework layering (e.g. Prospec's CLI layer structure).
...
- **Rule Source**: Downstream project's `prospec/CONSTITUTION.md` and `prospec/ai-knowledge/_conventions.md`.
- **Checks**:
...
- **Rule Source**: Downstream `prospec/index.md` and module map.
- **Checks**:
  - Are all impacted callers and downstream consumers identified in the Call Chain and Affected Modules table?
