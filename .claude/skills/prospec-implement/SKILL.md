...
description: "Implementation - Execute tasks from the task list, implementing features one by one — driven by tasks.md, not free-form coding without a task list. Triggers: implement, start coding, write code, 實作, 開始寫程式, 寫程式, 開始實作"
---
...
- `prospec change status implemented` refuses without a fresh green `test_attempt`; remediation per [`references/project-test-runner.md`](references/project-test-runner.md)
- Error handling
...
- **NEVER** treat checkboxes as test evidence — on a `prospec change status implemented` refusal, run `prospec check --record-tests --change <name>`
