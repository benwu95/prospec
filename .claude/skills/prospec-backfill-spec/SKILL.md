...
- **NEVER** write the backfill draft into `prospec/specs/features/` — backfill only stages `.prospec/changes/[name]/backfill-draft.md`; `prospec archive` (the CLI) stays the sole writer of the trust zone, and promotion is manual via the forward path
