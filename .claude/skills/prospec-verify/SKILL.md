...
- on **S/A** atomically advances `status: verified` (the gate `/prospec-archive` looks for); on
  **B/C/D** leaves `status` unchanged — state in the report that the change is NOT verified, list
  ...
  `quality_log` keeps the earlier S/A entry, so neither `status` nor `metadata-completeness` records
  this grade. That combination is NOT archivable: say so in the report and resolve the WARN/FAIL
