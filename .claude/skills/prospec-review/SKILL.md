...
Apply `references/review-format.md`. In short: **critical** blocks the loop and is auto-fixed; **major** does not block (proposed, passed to verify as WARN, never counted in verify's grade); **minor** is recorded but does not block, is not auto-fixed, and is not surfaced to verify's grade.
...
> **Clean review (0 findings)**: The CLI records this round's counts and clean sentence at merge time; `change log` only closes the round and audits self-reported counts.
...
1. **Every round** — including a **review-clean** round (0 critical / 0 major) and **circuit-breaker tripped rounds** — close the round via `prospec change log --skill prospec-review --result PASS|WARN [--warning "circuit breaker tripped: <type>"|"<unresolved item>"]` (Bash; result `PASS` when clean, `WARN` when unresolved majors/FAIL or circuit-breaker trips carry forward). The CLI records this round's counts and clean sentence at merge time; `change log` only closes the round and audits self-reported counts. A clean review that records nothing is indistinguishable from a review that never ran.
...
- [ ] a `prospec-review` `quality_log` round-close entry recorded (every round, incl. review-clean) and the review baseline stamped via `prospec check --record-review --graded-by <context>`
...
- **NEVER** record a review round after a `prospec review merge` test refusal — follow its remediation
