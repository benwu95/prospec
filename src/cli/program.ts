...
/**
 * Build the program, parse argv, and dispatch errors. Exit status is carried on
 * `process.exitCode` (set by command actions and by `handleError`), NOT by
 * calling `process.exit()` — so this runs safely in-process (the e2e suite
 * drives it directly) as well as from the `index.ts` entry point.
 */
