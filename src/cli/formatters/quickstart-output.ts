...
export function formatQuickstartOutput(
  ...
  for (const step of result.steps) {
    if (step.status === 'skipped') {
      lines.push(`${pc.dim('○')} ${sanitizeTerminal(step.name)} ${pc.dim('(already done, skipped)')}`);
    } else {
      lines.push(`${pc.green('✓')} ${sanitizeTerminal(step.name)}`);
    }
  }
  ...
  lines.push('');
  lines.push(
    `${pc.dim('→')} Scaffold ready. In your AI agent, run ${pc.cyan(`\`${sanitizeTerminal(result.nextStep)}\``)} to localize triggers, sync, and generate AI Knowledge.`,
  );
