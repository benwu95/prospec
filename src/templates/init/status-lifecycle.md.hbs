...
- **`prospec-archive`** archives ONLY `verified` changes; any earlier status is refused (verify to S/A first). Its Entry Gate is enforced by `prospec archive` itself, which **refuses** on a `metadata-completeness` FAIL, on any of the three provenance checks FAILing (see Provenance audit scope), or until affected-module Knowledge is synced — the **backstop** whose prevention point is the verify S/A commit prompt (`--allow-incomplete` exempts the completeness condition only, for a pre-schema record) — then archive sets `archived`.
