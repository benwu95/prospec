...
- **`prospec-implement`** sets `implemented` only when every `tasks.md` **code-task** checkbox is done (unchecked `[M]`/`[V]` tasks are reminders, not blockers) — `prospec change status implemented` **refuses** otherwise, distinguishing "implemented, awaiting verify" from "tasks planned".
