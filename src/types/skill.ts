...
export interface SkillConfig {
  ...
  triggers: string[];
  /**
   * English negative-scope baseline: short phrases naming what this skill is NOT
   * for. The English boundary sentence already lives in `description`; this list
   * is only the translation source `prospec agent triggers` scaffolds under
   * `skill_exclusions`, so it is never rendered on its own.
   */
  exclude?: string[];
  /** Skill type categorization */
  type: SkillType;
...
export const SKILL_DEFINITIONS: SkillConfig[] = [
  {
    ...
    description: 'Explore - Requirement exploration, problem investigation, and solution comparison partner — clarifies before a Story exists, not the Story writer itself.',
  ...
  },
  {
    ...
    description: 'New Story - Create change requests by guiding User Story and acceptance criteria definition — the SDD entry station, not a generic issue or ticket writer.',
  ...
  {
    name: 'prospec-plan',
    description: 'Plan Implementation - Convert User Story into technical implementation plan (plan.md) and change specification (delta-spec.md) — the SDD plan station, not ad-hoc architecture discussion.',
  ...
  {
    name: 'prospec-design',
    description: 'Design Phase - Generate visual and interaction specs from proposal (Generate Mode) or extract specs from existing design tools (Extract Mode). Supports pencil/Figma/Penpot/HTML platforms. UI scope only — not architecture design (that is the plan station).',
  ...
  {
    name: 'prospec-tasks',
    description: 'Break Down Tasks - Decompose implementation plan into an actionable task checklist (tasks.md) — the SDD station after plan, not a generic todo tool.',
    triggers: ['break down', 'task list', 'work items', 'how to split'],
  ...
  {
    name: 'prospec-ff',
    description: 'Fast-Forward Planning - Generate complete planning artifacts in one pass (Story → Plan → Tasks) — for clear requirements, not exploration.',
  ...
  },
  {
    ...
    description: 'Implementation - Execute tasks from the task list, implementing features one by one — driven by tasks.md, not free-form coding without a task list.',
  ...
  {
    name: 'prospec-verify',
    description: 'Verify Implementation - Run 5+1 dimension audit (tasks, spec compliance, constitution, knowledge-implementation consistency, tests, design consistency) and assign quality grade (S/A/B/C/D) — the SDD gate before archive, not a plain test run.',
  ...
  },
  {
    ...
    description: 'Generate AI Knowledge - Read raw-scan.md, analyze project structure, autonomously decide module boundaries, and produce Recipe-First module READMEs and index — first-time seeding, not the incremental update after a change.',
  ...
  },
  {
    ...
    description: 'Archive Changes - Archive completed changes, generate summary, sync requirements to feature specs, and gate archiving on Knowledge sync — the SDD terminal station, not git tagging or a release.',
  ...
  },
  {
    ...
    description: 'Incremental Knowledge Update - Parse delta-spec.md to identify affected modules, scan source code, and update module README, index.md, and module-map.yaml incrementally — after a change, not the first-time knowledge scan.',
  ...
  },
  {
    ...
    description: 'Quickstart Onboarding Finisher - localize skill triggers, re-sync agent config, prepare the knowledge scan, and chain into knowledge generation — runs once after `prospec quickstart`, not a per-session step.',
  ...
  },
  {
    ...
    description: "Prospec Version Upgrade Finisher - after `prospec upgrade` records the version, syncs agents, and reports gaps, work through the report's docs inventory: update init-created files whose format drifted and create missing ones (asking consent per file), then localize triggers for newly-added skills and re-sync — version upgrades only, not first-time onboarding.",
