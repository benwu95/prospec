...
> **2026-06-06 證據驗證快照**。對照部署 skills、`src/`、tests、reference 格式、`.prospec/archive/`、git log。判定原則：證據優先，無證據即判未完成。
...
| 分層 | 項目 |
|------|------|
| P0 補課 | BL-003（無 Entry/Exit Gate）、BL-004（`change.ts` 無 `scale`）、BL-019（無 Output Contract）、BL-020（無 `[STABLE]/[DYNAMIC]`） |
...
| Phase 4（2026 H2 新增） | BL-027 ~ BL-035 全部未開始 |
| OPT | A2、A4、B1、B2、B3、B6、D2、D4、D5、D7、D8、D9 未做；A1/D1/D3 **PARTIAL**（有雛形）；A3/C/D6 未逐項驗證 |
| Bug | **BUG-001** Open：`detector.ts` 未優先讀 `.prospec.yaml`、未排除 `node_modules` |
...
**驗收標準**：
- [ ] Constitution 每條 = 嚴重度（MUST/SHOULD/MAY）+ 對應 verify check
- [ ] `prospec init` 依 tech stack 產 3-5 條引導式規則範例（解 OPT-B1）
- [ ] verify 依規則嚴重度分級回報（MUST 違反=FAIL, SHOULD=WARN）
- [ ] （選）規則可映射至 CWE / 合規框架
