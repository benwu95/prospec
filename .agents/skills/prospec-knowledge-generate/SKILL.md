...
- **Project Section Extensions**: the convention's preserved registry is the only authority for registered extension IDs, headings, applicability, requiredness, visibility, and content format — not `.prospec.yaml` and not an inlined skill template. Emit a registered extension only with its section markers inside the README user block; preserve both marked extensions and freeform user notes when revising an existing README.
