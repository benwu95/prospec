...
- `Docs inventory:` — one line per init-created doc, `✓ <path> (template: <hbs>)` when present or
  `✗ <path> — MISSING (template: <hbs>)` when absent. This section is Step 2's authoritative scan
  scope: it is derived from the same registry `prospec init` creates from, so it can never miss a
  file init would create.
...
### Step 2: Refresh init-created docs (inventory → create/diff → consent)

`prospec upgrade` deliberately does NOT touch any doc `prospec init` created — those may carry user
edits, so updating their format (or creating a missing one) requires consent. Do it here:
...
- [ ] every doc in the report's `Docs inventory:` was handled — present docs diffed against their listed template and updated only on consent, MISSING docs offered for creation and created only on consent (or templates were unavailable / the inventory section was absent, and the step was skipped with a note)
...
| User declines creating a MISSING doc | Leave it uncreated; record it as declined in the Output Summary (the next upgrade will list it again) |
