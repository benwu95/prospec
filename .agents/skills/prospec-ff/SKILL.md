---
name: prospec-ff
description: "Fast-Forward Planning - Generate complete planning artifacts in one pass (Story → Plan → Tasks) — for clear requirements, not exploration. Triggers: fast-forward, ff, all at once, 一次到位, 快轉"
---
...
| Step | Action |
|------|--------|
| Station | Enter the `prospec-tasks` station (Station Transition Protocol): read its `SKILL.md` — the path `prospec status` prints — and run its phases there: `prospec change tasks` scaffold (including the quick `story → tasks` transition), decomposition per [`references/tasks-format.md`](references/tasks-format.md), and the Task Contract Verification with its **Physical Receipt Verification Protocol** ([`references/tasks-verifier-rubric.md`](references/tasks-verifier-rubric.md)) |
| Receipt | Apply the **Physical Receipt Verification Protocol** to the verifier report: a readable regular file exists with `size > 0`; when a completion claim precedes the write, inspect abstract subagent lifecycle state or transcript logs and await it; a crash, timeout or spawn failure takes the station's explicit, disclosed Harness Degradation path; never fabricate a mock report or a synthetic PASS (zero-mock) |
