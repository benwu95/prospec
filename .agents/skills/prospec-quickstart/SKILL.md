...
description: "Quickstart Onboarding Finisher - localize skill triggers, re-sync agent config, prepare the knowledge scan, and chain into knowledge generation — runs once after `prospec quickstart`, not a per-session step. Triggers: quickstart, onboard, bootstrap, get started, 快速開始, 初始化, 上手, 開始使用"
---
...
1. Run `prospec agent triggers` (Bash) to get a ready-to-translate scaffold with two blocks — `skill_triggers` (invocation words) and `skill_exclusions` (short phrases naming what a skill is NOT for) — listing only the skills still missing an entry in each, each with its English baseline sourced from the CLI (authoritative; never grep a deployed SKILL.md, whose frontmatter already merges custom words); translate both blocks and write both back with `--write`
