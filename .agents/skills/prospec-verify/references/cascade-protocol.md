...
1. **Step 1 [LOAD]** — Run `prospec status`, then load the station it names: read the `SKILL.md` at the fallback path it prints — on every transition and re-entry, never from memory. If that read does not yield the instructions, stop and name what is missing.
