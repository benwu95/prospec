...
rule (see project Constitution / `AGENTS.md`). Talk to the user in their language; write the artifacts in English.
...
- It lives under `.agents/skills/` and is version-controlled via an explicit
  `!.agents/skills/release/` exception in `.gitignore` (the repo otherwise tracks only
  `.agents/skills/prospec-*`).
