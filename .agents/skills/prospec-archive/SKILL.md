...
- **Review, test and delta-spec provenance (machine-checked)**: from the same `prospec check --json`, read the `review-provenance`, `test-provenance` and `delta-spec-provenance` checks for this change. **Any FAIL → do not archive** — the code this change is about to graduate into `prospec/specs/features/` is not the code its review round and test run actually saw. A *stale* finding has two possible causes and **the finding cannot tell you which** — the digest is one hash over HEAD plus the working tree, so a post-verify code edit and the verify S/A commit produce the same message. You decide: if code changed after the review round, re-run `/prospec-review` then `/prospec-verify`; if the only movement since the baseline was the verify S/A commit itself, re-record both (`prospec check --record-review` and `prospec check --record-tests`) **after** committing. When unsure, re-review — re-recording a baseline no review has seen is exactly the bypass this item exists to close. An *absent* review always means re-run `/prospec-review`. `skipped` is not a FAIL (an unavailable source is honest). Without this item the gate that writes the permanent record asserted nothing about whether any review had seen the final code. **When that re-run of `/prospec-verify` does not reach S/A, this change is not archivable** — a change already at `verified` keeps that status whatever the new grade is, so `status` will not say so — but `metadata-completeness` will: for a `verified` change it reads only the LATEST `/prospec-verify` entry, so an earlier S/A no longer covers a B/C/D re-verify. Resolve the WARN/FAIL items until a re-run grades S/A; never archive on the strength of a `status` the latest verify did not earn. A **`delta-spec-provenance`** FAIL is a different fact from the other two and has its own remedy: the `**Spec:**` blocks this phase is about to copy VERBATIM into the trust zone are not the ones the review round saw. That happens when review corrects a REQ's behavior and the correction is folded into the code but never back into the landing block — the other two checks stay green throughout, which is exactly the blind spot this one closes. Fix the block, not the code, then re-record with `prospec check --record-review`. It PASSES without comparing for a scale that carries no delta-spec and for a backfill proven by `backfill-draft.md`; only a missing `.prospec/changes/` makes it `skipped`. (The CLI is required — the probe STOPs before this gate when the engine is missing.)
...
0. **Start from the CLI's FIVE worklists** — they cover different failures, so no subset is the whole job:
   - **Graduation worklist** (`pendingConvergence`) — every REQ whose body the CLI deliberately did NOT replace (no `**Spec:**` block, or no body at all — that block is defined in the delta-spec-format reference `/prospec-plan` reads, not vendored here). Those REQs still carry their pre-change body (or a bare title) and need converging.
   - **Dropped behavior** (`droppedBehavior`, BLOCKING) — every REQ whose body a `**Spec:**` block DID replace, listing the existing `WHEN/THEN` bullets the replacement omits and which the delta-spec did NOT declare deliberate. A REQ absent from the graduation worklist landed its authored text, but "landed" does not mean "lost nothing": the block replaces the WHOLE body, so anything it failed to restate left the trust zone here. The spec was left unwritten: restore each bullet into the block, or list it under `**Dropped:**`.
   - **Refused REQs** (`refusedRequirements`, BLOCKING) — every REQ whose landing block was cut short by a label the delta-spec template does not own. The fragment was never trusted, so the spec was left unwritten. Fix the block the refusal names — for a MODIFIED entry that is `**Spec:**`, for an ADDED entry with no `**Spec:**` block it is `**Description:**`/`**Acceptance Criteria:**`. No declaration releases a refusal.
   - **Acknowledged drops** (`acknowledgedDrops`) — bullets whose exact text the delta-spec declared it does not carry forward, retirement and rewrite alike. They landed; confirm each against the merged file, because the declaration records that you looked, not that the behaviour was dropped.
   - **Stale declarations** (`staleDeclarations`) — declared bullets that were not dropped, which means the delta-spec describes an older body than the spec now has. Re-read the merged file before graduating.
1. Read **every requirement this change graduates** — one REQ at a time, from the **merged Feature Spec on disk**: `prospec spec show <feature> --req <ids>` (Bash). **Graduation key by scale** names that set: `standard`/`full` → the delta-spec's REQ ids; `backfill` → delta-spec (same path — REQ + Story; feature-slug REQ ids route by `**Feature**` as usual); `quick` → the proposal's **Spec Impact** section, applied manually (no delta-spec for the CLI to route; when the Entry Gate diagnosed no spec impact, skip graduation entirely — the summary.md diagnostic is the record). The worklists above do **not** define that set — every one of them is an EXCEPTION report, so a REQ that landed cleanly with a complete `**Spec:**` block appears in none of them while still needing its wording converged (and a `quick` change has no routes at all, which makes all five structurally empty). Read each graduating REQ, then use the worklists to see which of them ALSO need a kept body converged, a dropped bullet restored, or a refusal fixed. Never substitute the delta-spec entry or a worklist's own wording for the spec text — a body the CLI kept, or one it replaced, reads correctly only in the file it now lives in — and reading the whole spec is still the wrong move (tens of thousands of tokens of unrelated capability record)
