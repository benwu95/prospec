...
Emit a `ConstitutionRule` (RFC-2119 severity form) so `/prospec-verify` can grade it:
...
| zero-yield lens | did `prospec learn yield` mark a review lens `retire` / `review` — `invocation_source: declared`, with at least `min_invocations` declared runs? | the report (`--json`: `thresholds`, `consecutive_zero_changes`, `declared_invocations`, `last_yield_change`) — a rows-proxy lens is `keep` by construction and is not evidence |
...
- **Lens retirement**: lands in `_playbook.md` as a `Lens scope` entry naming the lens, the yield evidence and the approval — it tells the reviewer which conditional lens to stop loading for this project; the shipped lens list is prospec's, so no skill or reference file is edited.
