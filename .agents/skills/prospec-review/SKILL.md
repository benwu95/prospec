...
2. **At loop convergence** (review-clean or escalation), run `prospec check --record-review --graded-by <fresh-subagent|in-session> [--executor <label>]` — it code-computes the reviewed change's digest and writes `review_provenance` (with the declared grading context and, when given, your executor label — one your project's `.prospec.yaml` `executors` declares, when it declares any) to `metadata.yaml`. This is the baseline the `review-provenance` drift check compares against.
...
- [ ] a `prospec-review` `quality_log` entry recorded (every round, incl. review-clean) and the review baseline stamped via `prospec check --record-review --graded-by <context> [--executor <label>]`
