...
const certified = { recorded_exit_code: 0, recorded_command: 'test', recorded_attempt_id: 'a1', attempt_id: 'a1', attempt_outcome: 'passed', attempt_exit_code: 0, attempt_before_digest: 'same', attempt_after_digest: 'same' };
const test = (over = {}) => ({ ...review(), command_unavailable_reason: null, changes: [{ ...change, ...certified, ...over }] }) satisfies TestProvenanceSource;
describe('evidence validity', () => {
  ...
  it.each(['running', 'failed', 'unprovable', 'timeout'])('does not restore an older PASS after %s', (outcome) => {
    expect(evaluateTestProvenance(test({ attempt_outcome: outcome })).result.status).toBe('fail');
  });
  it('skips an unavailable attempt with its actual reason without restoring older PASS (V-265-1)', () => {
    const result = evaluateTestProvenance(test({ attempt_outcome: 'unavailable', attempt_exit_code: undefined, attempt_command: 'missing-test', attempt_reason: 'spawn ENOENT' }));
  ...
  it('reports actual uncertified attempt diagnostics (V-265-2)', () => {
    const result = evaluateTestProvenance(test({ attempt_outcome: 'timeout', attempt_exit_code: undefined, attempt_command: 'node test.js', attempt_reason: 'timed out after 10 ms', attempt_signal: 'SIGKILL' }));
  ...
  it('keeps durable failure before command-unavailable and backfill skips', () => {
    const source = test({ recorded_exit_code: 1, attempt_outcome: 'unavailable', attempt_exit_code: undefined, scale: 'backfill', backfill_draft_present: true });
