...
| **tests** | vitest, memfs, unit, integration, contract, e2e, knowledge-format, skill-format, token-corpus, drift, lessons-harvest, mcp-server, in-memory-transport, cli-first, station-commands | 測試, 單元測試, test suite, 驗證, vitest | Active | 4-layer test suite — 141 files, 3,120 tests (unit 2232 + contract 777 + integration 45 + e2e 66). Validates every module — format contracts, the cli-first probe + station-command contracts, the drift engine, token corpus, and the MCP protocol over in-memory transport. | Quality gate — validates all layers with pyramid coverage | types, lib, services, cli, templates |
