...
| **tests** | vitest, memfs, unit, integration, contract, e2e, knowledge-format, skill-format, token-corpus, drift, lessons-harvest, mcp-server, in-memory-transport, cli-first, station-commands, skill-invocation, factual-counts, public-docs | 測試, 單元測試, test suite, 驗證, vitest | Active | 4-layer test suite — 231 files, 5,462 tests (unit 3911 + contract 1313 + integration 83 + e2e 155). Validates every module — format and public-document contracts, the cli-first probe + station-command contracts, the drift engine, token corpus, and the MCP protocol over in-memory transport. | Quality gate — validates all layers with pyramid coverage | types, lib, services, cli, templates |
