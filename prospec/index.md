...
| **tests** | vitest, memfs, unit, integration, contract, e2e, knowledge-format, skill-format, token-corpus, drift, lessons-harvest, mcp-server, in-memory-transport | 測試, 單元測試, test suite, 驗證, vitest | Active | 4-layer test suite — 77 files, 1,840 tests (unit 1204 + contract 575 + integration 18 + e2e 43), incl. token-corpus + startup-loading-baseline + lessons-harvest fixtures + INIT_DOC_REGISTRY drift guards (init⇄registry equality, derivation bindings, template renderability); drift/reader tests run on real temp dirs (fast-glob/git bypass memfs); MCP protocol tested over in-memory transport, never a spawned daemon | Quality gate — validates all layers with pyramid coverage | all |
