...
| **templates** | handlebars, hbs, skills, agent-configs, recipe-first, loading-rules, references, change, stable-prefix | 模板, 範本, handlebars, template engine, resources, 穩定前綴 | Active | Handlebars template library — 13 skills + 1 shared partial, 17 references, 1 agent-config, 4 change, 13 init/steering/knowledge (49 `.hbs`, English-only); skill Startup Loading is static-first with `[STABLE]/[DYNAMIC]` markers | Pure resources — no logic, consumed by lib/template.ts | — |
| **tests** | vitest, memfs, unit, integration, contract, e2e, knowledge-format, skill-format, token-corpus | 測試, 單元測試, test suite, 驗證, vitest | Active | 4-layer test suite — 34 files, 711 tests (unit 319 + contract 350 + integration 15 + e2e 27), incl. token-corpus + startup-loading-baseline fixtures | Quality gate — validates all layers with pyramid coverage | all |
