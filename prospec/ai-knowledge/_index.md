...
| **services** | init, knowledge, change, archive, agent-sync, spec-sync, product, feature-map, triggers, language, measure, check, mcp, serve | 服務, 業務邏輯, business logic, execute pattern, use case, 量測報告, 漂移檢查, 真相層 | Active | Business logic — 14 services with `execute()` pattern, incl. init language selection, the quickstart orchestrator (init + agent-sync, skip-completed), the upgrade orchestrator (records the prospec `version` in `.prospec.yaml` via comment-preserving in-place merge + re-runs agent sync + best-effort refreshes raw-scan.md + report incl. config-field nudges from a curated registry, with interactive fill on a TTY; writes no curated docs — only the regenerable raw-scan.md; curated-doc format updates are the consent-gated /prospec-upgrade skill's job, BL-044), trigger synthesis, Recipe-First knowledge generation, deterministic multi-language raw-scan (11-language manifest detection), measurement-report loading, drift-check orchestration with --init-ci scaffold (8 checks incl. readme-counts, BL-043), and the read-only MCP server (8 resources + 2 tools, per-request reads, stderr-only diagnostics; BL-042 added the `knowledge://feature-map` + `spec://product` entry/index resources); archive task stats consume lib/task-markers; change-plan/change-tasks share the services/change-resolver helper; archive spec-sync is `$`-pattern-safe with feature-slug path containment + non-atomic-move rollback + forwarded knowledge warnings, change-plan/tasks guard re-runs (`--force`) and advance status forward-only, init writes its config marker last; archive `syncFeatureMap` is the sole writer of `feature-map.yaml` (bootstrap-once + no-clobber, co-located with product.md regen) (16 files, 4,410 lines) | Isolates business logic from I/O layer, enables testability | types, lib |
...
| **tests** | vitest, memfs, unit, integration, contract, e2e, knowledge-format, skill-format, token-corpus, drift, lessons-harvest, mcp-server, in-memory-transport | 測試, 單元測試, test suite, 驗證, vitest | Active | 4-layer test suite — 74 files, 1,772 tests (unit 1159 + contract 554 + integration 16 + e2e 43), incl. token-corpus + startup-loading-baseline + lessons-harvest fixtures; drift/reader tests run on real temp dirs (fast-glob/git bypass memfs); MCP protocol tested over in-memory transport, never a spawned daemon | Quality gate — validates all layers with pyramid coverage | all |
