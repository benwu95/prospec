...
|--------|----------|---------|--------|-------------|-----------|------------|
| **types** | config, schema, errors, skill, change, zod, language, triggers, token-budget, measurement, scale | 型別, 結構描述, type definitions, 錯誤類別, validation, 量測, 複雜度 | Active | Zod 4 schemas (incl. artifact_language/skill_triggers/measurement report/change scale), error hierarchy, skill definitions with trigger baselines, Constitution rule types (8 files, 657 lines) | Leaf module with zero internal deps — all others import from here | — |
...
| **services** | init, steering, knowledge, change, archive, agent-sync, spec-sync, product, triggers, language, measure | 服務, 業務邏輯, business logic, execute pattern, use case, 量測報告 | Active | Business logic — 11 services with `execute()` pattern, incl. init language selection, trigger synthesis, Recipe-First knowledge generation, measurement-report loading; archive task stats are kind-aware (3,601 lines) | Isolates business logic from I/O layer, enables testability | types, lib |
...
| **templates** | handlebars, hbs, skills, agent-configs, recipe-first, loading-rules, references, change, stable-prefix, entry-gate, scale, kind | 模板, 範本, handlebars, template engine, resources, 穩定前綴, 知識同步閘門, 複雜度適配 | Active | Handlebars template library — 13 skills + 1 shared partial, 17 references, 1 agent-config, 4 change, 13 init/steering/knowledge (49 `.hbs`, English-only); skill Startup Loading is static-first with `[STABLE]/[DYNAMIC]` markers; archive carries the knowledge-sync Entry Gate (BL-038); skills are scale-aware with a quick path and the task kind schema frozen in tasks-format (BL-004) | Pure resources — no logic, consumed by lib/template.ts | — |
| **tests** | vitest, memfs, unit, integration, contract, e2e, knowledge-format, skill-format, token-corpus | 測試, 單元測試, test suite, 驗證, vitest | Active | 4-layer test suite — 34 files, 757 tests (unit 325 + contract 390 + integration 15 + e2e 27), incl. token-corpus + startup-loading-baseline fixtures | Quality gate — validates all layers with pyramid coverage | all |
