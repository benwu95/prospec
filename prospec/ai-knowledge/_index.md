...
| **cli** | commands, formatters, commander, output, preaction, measure, check, strict | 指令, 命令列, command line, 終端, entry point | Active | CLI entry point — 10 commands + 11 formatters, parse → execute → format (23 files); `check --strict` maps FAIL to exit 1 | Thin I/O layer: no business logic, delegates to services | types, services |
...
| **tests** | vitest, memfs, unit, integration, contract, e2e, knowledge-format, skill-format, token-corpus, drift, lessons-harvest | 測試, 單元測試, test suite, 驗證, vitest | Active | 4-layer test suite — 41 files, 849 tests (unit 398 + contract 405 + integration 15 + e2e 31), incl. token-corpus + startup-loading-baseline + lessons-harvest fixtures; drift tests run on real temp dirs (fast-glob/git bypass memfs) | Quality gate — validates all layers with pyramid coverage | all |
