...
| **Progressive Disclosure（L0/L1/L2）** | L0=`_index.md`+`_conventions.md`（每次必載，≤1,500 tokens）；L1=`modules/{name}/README.md`（按關鍵字命中載入，≤400 tokens/模組）；L2=原始碼（按需）。原則：低層不重複高層資訊。 |
