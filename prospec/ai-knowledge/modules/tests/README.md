...
> 4-layer Vitest suite (fast-glob/git bypass memfs — 137 test files, 2,988 tests (unit 2131, contract 748, integration 43, e2e 66)); tests every source module.
...
| `tests/contract/{knowledge,cli-output}-format.test.ts`, `init-doc-registry.test.ts`, `mcp-server.test.ts`, `language-policy-scope.test.ts`, `bundled-templates-sync.test.ts` | Output-format + registry + MCP-protocol pins via real `renderTemplate()` / InMemoryTransport; cross-document language-scope agreement; bundle ≡ `src/templates`. `knowledge-format` also pins raw-scan's disclosure block — bullet item-set, caps disclosed, empty placeholder, the fallback-exception sentence, and order-independence (two input orderings of one file list render identically — the same-input-twice form was a tautology). |
