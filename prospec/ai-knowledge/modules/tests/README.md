...
> 4-layer test architecture using Vitest + memfs — 85 test files, 2,079 tests (unit 1361, contract 637, integration 38, e2e 43)
...
| `tests/unit/services/knowledge-update.service.test.ts` | Incremental knowledge updates incl. in-place auto-block replace (no $-injection), collectAllModules case-insensitive, removal-wins, BL-043 feature-prefix resolution (REQ-MCP-* → feature.modules ∪ related_modules, mint guard skips an unresolved feature prefix, module-prefix fallback preserved), curated columns rendered from module-map (not blanked to —), and the execute()-level on-the-fly curated backfill index→module-map (no-clobber, before collectAllModules, idempotent) (42 tests) |
...
| `tests/unit/services/check.service.test.ts` | Drift orchestration — skipped-never-PASS (all 11 checks), init-ci hardening assertions (SHA pins, shell: bash, fence-proof compose), feature-map drift wiring (BL-040), mcp-readme-counts + metadata-completeness wiring |
