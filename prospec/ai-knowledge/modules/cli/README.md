...
> Thin I/O layer — Commander commands parse args → call one service → format output (65 files)
...
| `formatters/` | 29 `formatXxxOutput(result, logLevel)` modules (+ `sanitize.ts`, `escaping-notice.ts`) — stdout success, stderr errors; all station hand-offs and semantic-review guidance use host-neutral bare Skill identities, while host syntax stays in generated entry configs. Archive and review formatter invariants are documented in the service sub-modules; `status-output` prints the service's next-station reference map as `read:` lines after `action:`, sanitizing every value and deciding nothing itself. `escaping-notice.ts` is the one-line escaped-cell notice a table writer (`review merge`, `learn upsert`) prints only when `escapedCells > 0`, wording from `types/cli-help`'s `ESCAPING_RULE_TEXT`; the six agent-called commands mount `renderCommandHelp(COMMAND_HELP_SPECS[…])` via `addHelpText('after', …)` (`status`, `change log`, `review merge`, `verify record`, `learn upsert`, `spec show`). |
