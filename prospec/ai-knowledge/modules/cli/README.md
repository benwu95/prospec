...
| `src/cli/formatters/upgrade-output.ts` | formatUpgradeOutput() — version delta + interactively-resolved nudge confirmations + config-field nudges / skills-missing-triggers / up-to-date + `Docs inventory:` section (fixed parse-friendly lines the /prospec-upgrade skill consumes as its scan scope: `✓ <path> (template: <hbs>)` / `✗ <path> — MISSING (template: <hbs>)`, paths through `sanitizeTerminal`) + next-step hint |
