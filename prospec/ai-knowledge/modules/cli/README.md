...
> Thin I/O layer — Commander commands parse args → call one service → format output (66 files)
...
| `commands/` | 28 `registerXxxCommand(program)` files: init, quickstart, upgrade, print-template, knowledge (init/update/verify), agent (sync/triggers), config, change (story/plan/tasks/log/status/scale/progress/auto-draft; story includes `--freeze-scenarios`/`--amend-scenarios`), status, spec show, archive (+`finalize`), review merge, verify (record and `context --change`), learn (upsert/yield), validate, measure (local session logs, projection mode), check, mcp — parse flags → **`await import()` the service/formatter inside the action** → format (registration stays service-free so unused commands' deps never load) |
| `formatters/` | 29 `formatXxxOutput(result, logLevel)` modules (+ `sanitize.ts`, `escaping-notice.ts`, `test-gate-output.ts` — the one `tests: not-adjudicated` WARN line `change status` and `review merge` both print; `error-output` appends `ESCALATE_TO_HUMAN` + trigger + `count / threshold` for a `TestGateError` whose breaker tripped) — stdout success, stderr errors; all station hand-offs and semantic-review guidance use host-neutral bare Skill identities, while host syntax stays in generated entry configs. Archive and review formatter invariants are documented in the service sub-modules; `status-output` leads with the canonical skill identity as `action:`, prints any resolved skill file as a separate `fallback:`, then the service's reference map as `read:` lines — sanitizing every value, naming no host and deciding nothing itself. `escaping-notice.ts` is the one-line escaped-cell notice a table writer (`review merge`, `learn upsert`) prints only when `escapedCells > 0`, wording from `types/cli-help`'s `ESCAPING_RULE_TEXT`; the six agent-called commands mount `renderCommandHelp(COMMAND_HELP_SPECS[…])` via `addHelpText('after', …)` (`status`, `change log`, `review merge`, `verify record`, `learn upsert`, `spec show`). |
...
- `registerXxxCommand(program)` — 28 registrars; `formatXxxOutput(result, logLevel)` — 29 formatters (+ the `formatEscapingNotice(escapedCells)` helper); `handleError(err, verbose)` → stderr
