...
| File | Purpose |
|------|---------|
| `change.ts` | Change metadata contract — loose read + strict build views (incl. `NewQualityLogEntrySchema`), `BareModuleNameSchema`, `issue`, `ReviewProvenanceSchema` (optional `graded_by` + `executor` label); its registry half (statuses, scales, artifact matrix, gate/grade enums) is in the sub-module |
...
| `station.ts` | Station I/O schemas — `ReviewFindingSchema` (+ its `repro`/`evidence` half), `JudgmentDimensionInputSchema` (each entry declares `graded_by`, optional `executor`/`spend` self-reports), `LessonInputSchema`, review status groups (`REVIEW_*_STATUSES`, `normalizeReviewStatus`/`hasReviewStatus`), the `prospec learn yield` contracts (`LensYieldThresholdsSchema` defaults 5/3/0.1, `LensYieldStatSchema`, `LensYieldReportSchema`, `LENS_RETIREMENT_ACTIONS`) and the `prospec learn stats` contracts (`ExecutorStatSchema`/`ExecutorStatsReportSchema` — every count record keyed by the full grade/result/context registry — plus `EXECUTOR_STATS_REPORT_FILENAME`); `RELAYED_FIELD_MAX_CHARS` and the dimension/kind registries are in the sub-module |
