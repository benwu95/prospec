...
| `errors.ts` | `ProspecError` base + 18 error subclasses (incl. `InvalidTransitionError`, `PauseAtInvalid` — names the source and value and the valid stations, `TestGateError` — entrance, actual reason, target-scoped remediation, optional tripped `CircuitBreakerState`, `warningRecorded` for the warning-only partial outcome) |
