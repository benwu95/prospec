...
| `src/types/drift-report.ts` | DriftReportSchema for prospec-report.json — structural/semantic layering, 9 frozen check ids (incl. `dangling-prefix`, `feature-modules`, `readme-counts`, `review-provenance`), skipped-needs-reason rule, frozen knowledge-health field contract; `ChangeMetadataSchema` carries an optional `review_provenance {digest,date}` (the code-computed review baseline the `review-provenance` check compares against) |
