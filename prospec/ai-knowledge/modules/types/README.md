...
| `src/types/drift-report.ts` | DriftReportSchema for prospec-report.json — structural/semantic layering, 10 frozen check ids (incl. `dangling-prefix`, `feature-modules`, `mcp-readme-counts`, `review-provenance`, `metadata-completeness`), skipped-needs-reason rule, frozen knowledge-health field contract; `ChangeMetadataSchema` carries an optional `review_provenance {digest,date}` (the code-computed review baseline the `review-provenance` check compares against) |
...
- `DriftReportSchema` / `DRIFT_CHECK_IDS` — validates prospec-report.json; `DRIFT_CHECK_IDS` lists 10 ids (incl. `dangling-prefix`, `feature-modules`, `mcp-readme-counts`, `review-provenance`, `metadata-completeness`); semantic layer is literally `'not-checked'` (never gradable), `skipped` checks must carry a `reason`
