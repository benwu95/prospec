...
| `status.ts` | `SDD_STATIONS` order, incl. the `knowledge-update` station, the no-status design station, and the `promote` backfill entry; `STATION_SKILLS`; `WORKFLOW_REASON_CODES` (append-only, matched by value — `AWAITING_HUMAN_PLAN_SIGNOFF` and `PLAN_VERIFIER_PENDING` follow `ESCALATE_TO_HUMAN`) and `HUMAN_HALT_CODES` |
...
- `DRIFT_CHECK_IDS` — the 22 frozen drift-check ids
...
2. **Add a drift check id** — append to `DRIFT_CHECK_IDS` (frozen, additive) → declare its scope in `DRIFT_CHECK_SCOPES` and its draft scale in `auto-draft`'s table (both total Records, so the compiler asks) → wire it in the drift services.
