...
> Sub-module of [Template Library](./README.md) — the skill-template contract (17 skills, 8 partials, 30 references) and what `agent sync` deploys from it.
...
- Judgment-station templates (`prospec-review`, `prospec-verify`, `prospec-plan`'s verifier section, `references/candidate-evaluation`) stay model/harness-agnostic: route grading to "the strongest available tier", never a named model or harness (a contract test greps the rendered set against a forbidden-name list); stations with a grading-context recording sink instruct honest in-session self-declaration, the sink-less ones a prose disclosure.
...
- The reuse gate spans three references but has ONE definition: the two-condition "single-source bypass" critical lives only in `review-format.hbs` §Severity Criteria; `plan-verifier-rubric.hbs` dimension 5 and the `review-lenses-content.hbs` Maintainability row cite it BY NAME (a contract test forbids the definition fragment outside review-format). The rubric's own FLAWS trigger is deliberately wider (any unargued owner bypass, any path) and self-contained — never import review's narrower threshold into it. Shared titles `Reuse & Single-Source` / `Simpler Alternative` are compared across rubric, plan-format §8 and `prospec-plan.hbs` Phase 4/6 by rendered string, and `cascade-protocol.hbs`'s plan→tasks gate carries the dimension count — five sites move together. `plan-format.hbs` sits ~35 tokens under its 2500 budget (pinned by a contract test), so a new section there must be paid for by trimming an example.
