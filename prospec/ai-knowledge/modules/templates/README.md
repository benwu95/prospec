...
- `prospec-implement` Phase 4 and `prospec-review` Loop step 3 carry ONE test sentence each, pointing at the CLI refusal/remediation (`change status implemented` / `review merge` refuse without a fresh green attempt) and keeping the `project-test-runner` reference at its load point — the policy lives in the CLI; `review-format` documents the test-failure metrics and `circuit-breaker` its `persistent_test_failure` dimension (default threshold literal pinned to `DEFAULT_CIRCUIT_BREAKER_CONFIG`). Section-scoped contracts pin all four; the shipped-reference size anchor was raised for the two references. The `prospec-review` skill's Review-Provenance counts prose and Clean-review callout were likewise demoted to a one-line pointer — the CLI records this round's counts and the artifact-language clean sentence at `review merge` time, so `change log` only closes the round and audits self-reported counts.
...
- Variables are NOT compile-checked — a typo or `undefined` array yields silent empty output. `_knowledge-loading-rules.hbs` must carry a row per knowledge budget field (contract-asserted per per-project field, from the schema's keys; shipped skill/reference budgets have no row and both seeds omit them): an un-rowed field renders as an empty cell in every generated `index.md` and SKILL.md, and a fixture that hand-lists a subset of the budget cannot detect it.
...
- All templates are English-only (REQ-TEMPLATES-073); document language comes from the Constitution Language Policy — `entry.md.hbs` renders its scope from the six keys `entryLanguageContext` returns (`language_*` and `trust_zone_*`, spread by BOTH render sites — missing key → empty, not an error) and branches single-zone / two-zone on `language_single_zone` + `trust_zone_is_english`; the `_language-policy` partial and the graduation references read `trust_zone_language` from the same spread, and `review-format`'s Summary AND evidence prose follow the artifact language while enums/paths/`repro` stay English. That reference and `prospec-review`'s own Persistence paragraph both restate `lib/review-merge`'s identity rule — the skill body is read FIRST, so all three move together or the reviewer is taught the stale one. The delegated-payload ceilings avoid that shape entirely: ONE reference, both stations, numbers injected. Never quote literal mustaches in prose.
...
- Editing a shipped `.hbs` takes two steps: `pnpm bundle`, then sync from source (`npx tsx src/cli/index.ts agent sync`) — `pnpm exec prospec` resolves to the globally installed binary and silently deploys the released templates. `pnpm bundle` also regenerates `src/lib/bundled-templates.ts`, so a pure `.hbs` change makes **lib** source-touched too — stamp lib's knowledge (it surfaces as a `stamp-only` module in `prospec knowledge update --change`).
...
- The opt-in plan sign-off is the human's: the surfaces an agent acts from — `prospec-plan` / `prospec-ff` `## NEVER`, candidate-evaluation `## Selection Paths`, cascade-protocol — carry a NEVER rule against running `prospec change log --signoff`, or setting `PROSPEC_PAUSE_AT` to skip a pause, without an explicit human instruction (contract-pinned to exactly those four); descriptive surfaces (metadata-format, both lifecycle copies, config-example) name them without a rule. The no-pause path states that the agent NEVER asks the human to choose.
- The quickstart / upgrade localization step names BOTH `skill_triggers` and `skill_exclusions` (the scaffold prints two blocks) — a section-scoped contract pins the sentence, so dropping one key silently mis-describes the CLI.
- Dual-copy documents: status-lifecycle lives in BOTH `init/status-lifecycle.md.hbs` and `prospec/ai-knowledge/_status-lifecycle.md`, module-README conventions in BOTH `init/module-readme-conventions.md.hbs` and `prospec/ai-knowledge/_module-readme-conventions.md` — edit both. status-lifecycle's `## What each gate checks` AND `## Gates` are machine-compared between the copies — the only whole-section string equality, so prose elsewhere can diverge silently — and its `## Station order` chain, `## Light-scale artifact matrix` and `## Provenance audit scope` table are compared against `SDD_STATIONS` / `SCALE_FORBIDDEN_ARTIFACTS` / `PROVENANCE_AUDITED_STATUSES` in BOTH copies (`skill-format.test.ts`) — copy-to-copy agreement alone never proved either matched the code. The conventions pair's core block IS byte-compared; only its user-block registry is unguarded, so a one-sided edit THERE is silent. The unguarded prose is where ENGINE-behavior claims rot: the `## Gates` sentence describing what `metadata-completeness` reads survived a change that inverted that very judgment, in both copies and both shipped skills, until adversarial review caught it — a marker-pinned claim would have gone red instead.
