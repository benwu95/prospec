...
| `src/templates/agent-configs/entry.md.hbs` | Shared entry-config template — declares `artifact_language` (L0), lists per-skill Triggers, and surfaces in-progress `.prospec/changes/` at session start (workflow-order next step); wrapped in `prospec:auto`/`prospec:user` blocks so `agent sync` refreshes only the auto block and preserves user edits — same marker contract as `init/agents.md.hbs` (the init AGENTS.md stub) and `_index.md` |
