...
- A skill template that interprets a CLI refusal must distinguish the KINDS the CLI can report: `archive finalize` refuses the whole run (summary not overwritten — fix and re-run) AND refuses one reconciliation (a spec whose declared counter the body would zero — re-running never fixes it). Collapsing them sends the agent back to overwrite a summary that is already correct.
