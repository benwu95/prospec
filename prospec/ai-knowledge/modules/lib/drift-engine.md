...
- `collectBudgetOverrides` also credits the collection's own `commentBefore` to the block's FIRST entry: the YAML AST hangs a leading comment on the map, not on the key it introduces, so reading only the entry's three comment slots flags a justified first override forever. That credit is positional (index 0) — spread it to every entry and one comment justifies the whole block.
