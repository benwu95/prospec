...
- `runChecks(inputs)` + every `collect*` source, `computeChangeState` (NUL-delimited Git enumeration plus raw-byte final-file snapshot yielding digest and clean diagnostics; `computeChangeDigest`/`computeWorkingTreeClean` are thin wrappers so a caller needing both pays one capture), `changedPathsFromWorkTree` (working-tree changed paths over the digest scope — the pre-commit counterpart of the committed-range knowledge-sync gate) + `partitionDiffAttributedModules` (splits diff-attributed modules into REQ-acknowledged vs stamp-only via `moduleAttributor`), `computeDeltaSpecDigest`, `aggregateEscapedDefects`, `parseConstitutionRules`, `resolveTestCommand`/`runTestCommand`
...
- Drift findings are codepoint-sorted (`localeCompare` breaks byte-identity, tie-breaks included); an unavailable source → `skipped`, never a vacuous pass (`import-direction`: JS/TS ESM only). For canonical-doc-drift, the collector never throws (per-doc read/render is try/skip) and an absent canonical doc is also skipped.
...
- L2 sizing walks a module dir via the ONE `moduleKnowledgeFiles` helper — README **and** each sub-module `.md`. Staleness no longer does: it compares `last_src_commit` against the module's `last_verified` (module-map, stamped only by `knowledge verify`) **by UTC calendar day** — the stamp lands moments before the commit carrying both, so an instant compare would red every freshly-confirmed module. Absent `last_verified` = stale; `last_readme_commit` / `last_sub_module_commit` stay in the frozen report shape but no longer drive the verdict.
...
- NO enumeration in that collector may throw: it is an ARGUMENT to `runChecks(...)`, so one bad path costs every other verdict, not its own line — hence `readdirNamesOrEmpty` and `budgetedMarkdownFiles` rather than `scanDirSync`, which throws AND applies `SENSITIVE_PATTERNS` (a Feature Spec named `secret-*.md` would vanish — a budget failing OPEN). That walk follows its ROOT (a legitimately symlinked tree must still be measured) but never descends a symlinked sub-directory. Still unguarded and out of scope: `collectMarkdownLinks`/`collectReqDefinitions` abort the run on an EACCES dir or a `specs/features` that is a file.
- `test-runner.ts`'s argv[0] follows **libuv**, never PATHEXT (spawn cwd before PATH, entries unquoted). Unresolvable commands and a latest unavailable attempt skip with actual command/reason; a persisted non-zero exit still FAILs first. Collectors retain latest attempt diagnostics for the existing terminal sanitizer.
- All THREE provenance evaluators filter through `PROVENANCE_AUDITED_STATUSES` (`types/change.ts`, pinned against the lifecycle doc's audit-scope table): `implemented` **and** `verified`, so the verify→archive window is covered; `archived` is unreachable (bundle moved), not exempt. The former HEAD-based rule (PB-016) is superseded by `snapshot-v2` / `repository-inputs-v2`: equivalent staging/commit/amend preserves evidence. Legacy versions require real revalidation.
- `computeChangeDigest` hashes UNTRACKED contents too (fails closed) — a tool writing non-ignored inputs invalidates provenance. Declare disposable outputs in Git ignores; tracked outputs remain inputs.
...
- `collectBudgetOverrides` routes a shipped key to `ineffective` before any value check (the key is the finding); `evaluateBudgetOverrides` warns per entry under the same id. It also credits the collection's own `commentBefore` to the block's FIRST entry: the YAML AST hangs a leading comment on the map, not on the key it introduces, so reading only the entry's three comment slots flags a justified first override forever. That credit is positional (index 0) — spread it to every entry and one comment justifies the whole block.
- Snapshots reconcile status-observed additions with enumeration; nested-project porcelain paths are normalized to the project cwd before scope filtering. Snapshots frame path, kind, mode and bytes; unreadable/unsupported inputs fail closed. The assessment retains workflow facts and observes membership/config before and after collection, then rechecks before writes. Neither mechanism isolates transient change-and-restore or external/ignored dependencies.
- The contained read belongs to `knowledge-reader` ([lib](./README.md) Pitfalls) — drift-sources imports FROM it, never the reverse.
- `collectLanguagePolicyDrift` owns only the read: the verdict is `language-policy`'s `compareLanguagePolicy`, the same function `upgrade.service`'s stale signal calls — never re-implement the `**Description**:` extraction or the whitespace normalization here. It compares against `languagePolicyRule(scope).description` (passed in as a string — `language-policy` cannot import `constitution-rules`, which imports it), so a config language change alone flips an unchanged Constitution to `diverged`.
