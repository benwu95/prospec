...
The other 18 `.ts` are single-purpose helpers (`ls src/lib`); those with invariants appear in Pitfalls.
...
- Station engines decide, never re-derive policy: `verify-grade` has NO WARN exemption (`not-adjudicated` included — each spends grade A's budget); `lessons-ledger` counts DISTINCT source changes; `review-merge` never infers identity from a location — its (location, lens) fallback needs one side with no id, sees pre-round rows only, and drops any row it claims, moves or names by id.
