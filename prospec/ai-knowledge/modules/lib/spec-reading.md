...
2. **Change what a REQ BODY is** — edit `indexSpec`'s boundary predicate, then the archive merge's (see Pitfalls: the boundary has two owners).
...
- A REQ body ends where the archive merge ends it: another REQ heading, a heading at or above its own level (h1/h2 always, or an h1-level REQ would swallow `## Edge Cases` and the Change History table), or a `---` rule. That boundary has exactly TWO owners — here and `archive.service`'s in-place merge — and the contract test pins the pair.
