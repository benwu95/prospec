...
- **Section anchors match the HEADING, never the bare string.** Both insertion points — `## Edge Cases` for an ADDED requirement, `## Deprecated Requirements` for a REMOVED one — anchor at line start (`/^## Edge Cases[ \t]*$/m`). A feature spec routinely quotes its own structure, so those strings also sit in prose and inline code spans: `drift-detection.md` carries both roughly 110 lines above the real headings. A first-substring match lands there instead and splices the new block into the middle of another requirement's bullet, truncating it — and nothing reports it, because both worklists come back empty and the Change History row is still written.
- FUNCTION replacers (verbatim `$`) in every insertion branch, path-contained `**Feature:**` slug, no-clobber `feature-map.yaml`; NO auto knowledge-update (the skill + verify prompt own it).
