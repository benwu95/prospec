...
| `spec-show.service.ts` | `execute({cwd, feature, req, story})` — resolve one feature spec, then let `lib/spec-slices` decide what to quote |
...
3. **Add a narrow-read consumer** — call `lib/spec-slices`; never parse a spec in a service.
...
- Feature-name resolution goes through `readFeatureSpec` — containment, the `_archived*` exclusion and the resource-name guard in one place; an unresolvable name is a refusal that NAMES the specs that do exist, never a silent empty read.
