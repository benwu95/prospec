...
- `status.service` resolves the opt-in plan pause ONCE, before the per-change loop, from an injectable `env` (default `process.env`): an invalid setting rejects the whole command (clean state included) with `PauseAtInvalid`, while an unreadable or unparseable `.prospec.yaml` pauses and appends a reason naming the file — the one pause fact that is never swallowed with the rest of a broken config.
