...
- check.service keeps every side effect behind a flag (the pure path is read-only and byte-reproducible); it computes the change digest ONCE and passes it to both provenance collectors, and `--record-tests` checks every precondition (command, git, metadata) BEFORE spawning the suite, recording the POST-run digest so a suite that writes artifacts still converges. It re-reads metadata AFTER the run and merges `test_provenance` into the fresh document — a mid-run edit is preserved, and a file that stops validating mid-run records nothing (the stale snapshot never resurrects).
