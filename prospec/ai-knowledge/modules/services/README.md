...
Also: `quickstart` (init + agentSync), `agent-triggers` + `trigger-localization`, `measure`, `print-template`, `config-example` services. README/index **content** is skill judgment (`/prospec-knowledge-generate`) — no service generates it.
...
- spec-sync NEVER blanks an authored REQ body — only a `**Spec:**` block replaces one (ADDED may also fall back to Description+AC); everything else survives byte-identical and comes back in `pendingConvergence` (incl. REMOVED REQs whose active section still stands).
