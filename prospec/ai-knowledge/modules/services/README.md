...
> Business logic — one `execute(options) → Promise<Result>` per command + shared helpers (33 files)
...
| `verify-record` / `review-merge` / `learn` (upsert + yield) / `learn-stats` / `validate.service.ts` | Station bookkeeping — S/A/B/C/D grade (S/A appends quality_log AND advances `status: verified` in one write) plus the judgment evidence appended to `verify.md` when `--dimensions` carries it; every judgment verdict must declare its grading context (refused before write) and an in-session one caps the grade below S (`selfVerifiedCap` names the remedy); cumulative review.md table plus each finding's evidence block; ledger upsert + scoring + TTL; lens yield analysis (`executeYield` — thresholds from `.prospec.yaml` `learn.lens_thresholds`, corpus = `.prospec/archive` plus `--corpus` dirs; only declared `--lenses` runs count toward retirement); `learn-stats.service` — read-only per-executor statistics over archive `metadata.yaml` (enumerates through `learn.service`'s exported `listArchivedChangeDirs`, the ONE `--corpus` semantics; `readChangeMetadataLeniently` null → `skipped`; `--json` writes `executor-stats-report.json`); an `executor` on a verify verdict or `--record-review` is asserted against `.prospec.yaml` `executors` via `lib/config`'s `assertExecutorLabel` BEFORE any write (`verify record` reads config only when a verdict carries one); artifact verdicts |
...
| `check.service.ts` | Drift-check orchestration — collectors → evaluators → report (stamps `change_digest`); non-check modes (init-ci/record-review/record-tests/escaped-defects) return before the run (record-review also stamps the reviewer's self-declared grading context and executor label into `review_provenance` when supplied — absent otherwise, never blank); `--json` writes the report and `--auto-draft` drafts AFTER it, so drafting can neither discard the report nor move the exit code; every collector goes through a canonical resolver, never a re-derived path |
