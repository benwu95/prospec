...
- `raw-scan.service` classifies through `lib/module-detector`'s exported `isSourceFile` — never its own copy; the disclosure it renders is a fact about the file list, so `--raw-scan-only` reports it without running detection.
