...
| `verify-record` / `review-merge` / `learn` / `validate.service.ts` | Station bookkeeping — S/A/B/C/D grade (S/A appends quality_log AND advances `status: verified` in one write) plus the judgment evidence appended to `verify.md` when `--dimensions` carries it; every judgment verdict must declare its grading context (refused before write) and an in-session one caps the grade below S (`selfVerifiedCap` names the remedy); cumulative review.md table plus each finding's evidence block; ledger upsert + scoring + TTL; artifact verdicts |
