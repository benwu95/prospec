...
Which statuses `review-provenance` and `test-provenance` audit. `PROVENANCE_AUDITED_STATUSES` in `types/change.ts` is the **executable copy**, pinned against this table both ways; both gates read that one registry, so neither can cover a window the other does not.
