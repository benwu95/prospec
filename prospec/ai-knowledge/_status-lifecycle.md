...
- **`/prospec-verify`** sets `verified` ONLY at grade S/A. Grade B/C/D leaves `status` unchanged (stays `implemented`); fix the WARN/FAIL items and re-run. Its machine dimensions are adjudicated by `prospec check` (verdicts adopted verbatim), its judgment dimensions in fresh context; an unavailable engine yields `not-adjudicated` (S unreachable, A attainable). Division of labour: verify's Key Difference section.
...
- **`/prospec-review`** — sits **between `implemented` and `verified`** (adversarial review before verify — division of labour: verify's Key Difference); records `review_provenance` but does not change `status`.
