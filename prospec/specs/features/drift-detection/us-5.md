...
- WHEN `recorded ≠ current` AND `working_tree_clean` is `true`, THEN the stale finding names the cause as commit-induced (the baseline predates the current commit) and directs to re-record with `prospec check --record-review`, noting a full `/prospec-review` is needed only if code actually changed before the commit; WHEN `working_tree_clean` is `false` or `null`, THEN the finding keeps the code-changed wording; `evaluateTestProvenance` applies the same `working_tree_clean` signal identically, its commit-induced remedy pointing to `prospec check --record-tests` (the suite outcome is unknown after the commit, not a failure)
...
- WHEN an executor label is supplied and the declared vocabulary rejects it, THEN nothing is written; when it is accepted or no vocabulary is declared, THEN `review_provenance.executor` is written in the same document write

#### REQ-CLI-012: prospec check --record-review flag
`prospec check` adds `--record-review` (records the review baseline then exits) and `--change <name>` (targets record-review when multiple changes run in parallel), alongside `--json`/`--strict`/`--init-ci`; it also adds `--graded-by <fresh-subagent|in-session>` (validated against the two-value enum) which, with `--record-review`, records the review convergence's grading context into `review_provenance.graded_by`, and `--executor <label>` (a non-empty label, parsed by the shared executor parser) which, with `--record-review`, records the reviewer's executor label into `review_provenance.executor`; when the flags are absent, behavior is completely identical to the current one.
...
- WHEN `--executor <label>` is passed with `--record-review`, THEN the value is recorded into `review_provenance.executor`; an empty value is refused by the parser

#### REQ-TESTS-042: review-provenance engine tests
`evaluateReviewProvenance` six scenarios (absent/stale/fresh/backfill/outside-the-audit-scope/unavailable — the audited statuses themselves, and the `verified` cases in particular, are covered by REQ-TESTS-073), `computeChangeDigest` (temp git dir: changing `src`/`scripts`/docs content flips the digest, changing only `.prospec/`/report/generated does not), `collectReviewProvenance`, `check.service` injection + `--record-review` writes metadata + `--strict` FAIL → exit 1 + backfill skipped — mutation-verified. The stale branch additionally asserts the `working_tree_clean` message split: `true` yields the commit-induced remedy naming `prospec check --record-review`, while `false` and `null` keep the code-changed wording; `evaluateTestProvenance` carries the mirror assertions naming `prospec check --record-tests`; `computeWorkingTreeClean` is covered over a temp-git fixture in all three states (clean / dirty / capture-failure → null), and the message split is pinned by a revert-red mutation.
