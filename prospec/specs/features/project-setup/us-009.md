...
`prospec quickstart` chains init + agent sync (skipping already-completed steps), printing the agent-side next step; registered in `INIT_COMMANDS`, runnable before `.prospec.yaml` exists; `--name`/`--agents`/`--language`/`--trust-zone-language` pass through to init.
...
- WHEN `--trust-zone-language Y` is given, THEN it reaches init unchanged and `.prospec.yaml` records `trust_zone_language: Y`
- WHEN `prospec quickstart --help`, THEN the output lists `--trust-zone-language`
