...
- WHEN `_module-readme-conventions.md` is projected into the registry, THEN it remains canonical with `preserveUserContent: true`; every other current canonical doc leaves that flag absent, and consumers use the field rather than a filename special case
