...
`ProspecConfigSchema` adds an optional `artifact_language` (free-form string), an optional `trust_zone_language` (free-form string, the language of the trust zone — Constitution / README / index / `specs/product.md` / `specs/features/**` / knowledge base), `skill_triggers` (skill name → string array), and an optional `executors` (a non-empty array of non-empty label strings — the executor vocabulary `verify record` and `check --record-review` validate against; prospec assigns the labels no meaning). Neither language field carries a schema default: each resolver supplies English when the field is absent or blank, so a typed `ProspecConfig` literal never has to spell it.
...
- WHEN `executors` is absent, THEN the config validates and every executor value stays a free string
- WHEN `executors` is an empty array or contains an empty string, THEN validation fails (ConfigInvalid)
