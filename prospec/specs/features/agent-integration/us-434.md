...
getSkillReferences and SkillReference remain importable from agent-sync.service.ts as a compatibility facade. The facade derives its reference inventory from STATION_REFERENCES; it owns no separate skill-to-reference table.
- WHEN reading `getSkillReferences` / `SkillReference`, THEN both are services exports that tests can import
- WHEN agent sync regenerates a skill, THEN the facade's reference set equals the registry deployment projection and the pre-migration reference output-name set, while generated prose maps preserve existing phase and loading semantics.
...
AgentConfig's render-affecting fields live in AgentRenderFlags, including the closed skillContentLifecycle field whose rendered name is skill_lifecycle. GROUP_RENDER_FLAG_REDUCERS is mapped over keyof AgentRenderFlags with each reducer's input and output matching that field's type; mergeGroupRenderFlags applies this sole reducer registry. RENDER_FLAG_KEYS and its AssertNever exhaustiveness check continue to require every field.
- WHEN a render flag is added to AgentRenderFlags without a GROUP_RENDER_FLAG_REDUCERS entry or RENDER_FLAG_KEYS entry, THEN type checking fails instead of silently omitting its merge rule.
- WHEN merging surfacesSkillFrontmatter across members, THEN the result is true only if the member list is non-empty and every member is true; any false or empty group preserves the full registry.
- WHEN a group has a single member, THEN mergeGroupRenderFlags preserves that member's declared values.
- WHEN merging skillContentLifecycle, THEN the non-empty all-persistent-reattach group resolves persistent-reattach, the non-empty all-tool-output group resolves tool-output, and every other or empty group resolves unknown; order and duplicate members do not affect the result.
- WHEN services construct template contexts, THEN skill_lifecycle and its branch boolean derive from the merged field, never from an individual member or an independent lifecycle reducer.
