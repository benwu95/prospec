...
`agent-sync.service` `generateEntryConfig` renders `surfaces_skill_frontmatter` from the group-merged `AgentRenderFlags` (`mergeGroupRenderFlags` over all group members), not from any single member (`configs[0]`), reusing the single shared entry template.
