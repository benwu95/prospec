...
#### REQ-TEMPLATES-208: Verify Failure-Recovery Reference
...
- WHEN the Maintainability / DRY table is read, THEN it carries a row for re-implementing a knowledge-base-documented single-source helper (or an existing service's guard) instead of delegating to the owner, whose severity cell defers to the single-source bypass criterion in `review-format.md` by name — the path condition and the both-conditions rule are stated only there — and falls back to major (DRY) otherwise

#### REQ-TEMPLATES-210: Third-Party MIT Attribution
...
- WHEN a per-project budget field (one `TokenBudgetSchema` declares) is injected but no template row renders it, THEN a contract test fails — Handlebars renders the unknown variable as the empty string, so the guard is per field, not per row; the shipped fields are injected too but deliberately have no row
...
- WHEN `.prospec.yaml` does not override, THEN every rendered budget is its shipped default; after overriding any schema-declared field and re-syncing, the rendered number is the override
- WHEN `.prospec.yaml` carries any value for `skill_per_file` or `reference_per_file`, THEN the rendered loading table carries no Skill row and no shipped budget number — those budgets are not a project setting and are never rendered downstream
