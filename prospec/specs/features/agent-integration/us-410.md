...
Artifact-producing skills (new-story/plan/tasks/ff/design/archive/learn/knowledge-generate/knowledge-update, plus prospec-upgrade) load an explicit compliance directive via a shared partial (`{{> language-policy}}`) that points at the Constitution and hardcodes no language name. The directive is **path-scoped**: each generated document takes the language the rule assigns to its path — change artifacts and their archived summaries in the artifact language, the trust zone in the trust-zone language, both injected from the resolved scope — and states that one skill run may write both (archive writes `specs/_archived-history/` and `specs/features/**` in the same pass). The `spec-graduation` and `delta-spec-format` references and the `prospec-archive` skill describe the language of graduating REQ bodies the same way, through the injected trust-zone language, never a literal.
...
- WHEN the templates are rendered under a native trust-zone context, THEN the partial, `spec-graduation`, `delta-spec-format` and `prospec-archive` name that language for the trust zone and REQ bodies and state no English trust zone
...
`skill.ts` SKILL_DEFINITIONS `description` is the single source for each skill's description. The agent-sync per-skill render context passes in `skill_description` (`escapeYamlScalar`), and the skill `.hbs` frontmatter renders `"{{skill_description}} Triggers: {{trigger_words}}"` (not hardcoded); the CLAUDE.md/AGENTS.md registry and the SKILL.md frontmatter share the same source and do not drift. Both exits render the description through one `renderSkillDescription` helper, which appends ` Not for: <localized phrases>.` only when `.prospec.yaml` `skill_exclusions` names the skill.
- WHEN generating SKILL.md without localized exclusions, THEN the frontmatter description equals skill.ts `description` verbatim (round-trip after escaped-scalar parsing; equivalence contract test, mutation-verified)
- WHEN inspecting the verify description, THEN it reflects the current 5+1 dimensions
- WHEN `skill_exclusions` provides localized phrases for a skill, THEN the entry-config registry line and the SKILL.md frontmatter carry the same `Not for:` clause after the description and before `Triggers:`
- WHEN `skill_exclusions` is absent, THEN every generated file is byte-identical to the render without the key
...
Baseline trigger words (skill.ts) have no cross-skill substring or exact-dup collisions and are **prospec-specific** — bare generic terms that collide with CLI command names, with harness-built-in tools, or are common in general development conversation (check/change/upgrade/setup/done/critical/feedback/clean up/I want to/migrate version/version bump/review/tasks/design) are removed in favor of prospec-qualified phrases (e.g. `upgrade prospec`/`user story`/`quality check`/`finalize change`/`technical plan`/`code review`/`task list`/`UI spec`); this project's `.prospec.yaml` Chinese trigger words are likewise collision-free and equally converged. Each skill has ≥3 words (machine-enforced by REQ-TESTS-053).
...
- WHEN inspecting the baselines, THEN none of `review`, `tasks`, `design` appears as a bare trigger of any skill
...
- WHEN the declaration and the Constitution rule are compared, THEN their native/trust-zone path sets and both zone languages are identical
- WHEN `trust_zone_language` names a non-English language, THEN the declaration renders it for the trust zone
