...
`templates/skills/prospec-upgrade.hbs` runs the required CLI probe, executes `prospec upgrade --no-interactive`, and uses the report's `Docs inventory:` as its sole refresh scope. Present user-managed docs receive format-only diffs, canonical docs receive full-content diffs except canonical docs marked `preserveUserContent`, missing docs are offered through the safety net, root `index.md` may be enriched with current modules, stale seeded Language Policy wording is separately consent-gated, missing localized triggers are filled through the existing commands, and any resulting change is deployed through `prospec agent sync`.
