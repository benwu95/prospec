...
`templates/skills/prospec-upgrade.hbs` runs the required CLI probe, executes `prospec upgrade --no-interactive`, and uses the report's `Docs inventory:` as its sole refresh scope. Present user-managed docs receive format-only diffs, canonical docs receive full-content diffs except canonical docs marked `preserveUserContent`, missing docs are offered through the safety net, root `index.md` may be enriched with current modules, a Language Policy whose Description no longer matches the rule rendered for the project's resolved language scope is separately consent-gated, missing localized triggers are filled through the existing commands, and any resulting change is deployed through `prospec agent sync`. The skill states the trust-zone language as governed by `trust_zone_language`, never as fixed English.
...
- WHEN a present canonical/no-authored-content file without `preserveUserContent` differs from the rendered template, THEN show a full-content diff and replace it wholesale only after asking consent; if not consented, leave the file untouched
- WHEN a present canonical file is marked `preserveUserContent`, THEN show the template-owned format diff and, only after consent, migrate the generated/static format while preserving its user block; a marker-less legacy file receives an explicit no-clobber migration diff rather than a wholesale overwrite
...
- WHEN the report does not flag it, THEN the step never runs (a Description that matches the resolved scope is never touched)
- WHEN Step 3 offers an artifact language, THEN it states that the trust-zone language is governed separately by `trust_zone_language`, not that it stays English
...
`prospec upgrade`'s docs inventory tags each init-created doc with canonical and preservation classifications sourced from `INIT_DOC_REGISTRY` — the same subset the `canonical-doc-drift` check scopes over — and the report's `Docs inventory:` line surfaces the tags.
...
- WHEN the report lists `_module-readme-conventions.md`, THEN its inventory entry carries `preserveUserContent: true` and the formatted line carries a stable `[preserves-user-content]` marker alongside `[canonical]`
- WHEN another registry doc does not declare preservation, THEN its inventory entry does not carry the preservation marker
