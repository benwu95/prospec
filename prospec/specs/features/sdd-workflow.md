...
- WHEN summary generated, THEN includes User Story, REQ IDs, modules, statistics；任務完成率僅計 code task，`[M]`/`[V]` 分開列示（`N/M [M]/[V] (not counted)`）
- WHEN complete, THEN summary → `specs/history/` + auto-trigger knowledge-update (non-fatal)；delta-spec 缺席（quick 路徑）時 safety net 跳過——skill 層 Entry Gate（diff 路徑推導）為強制檢查點

#### REQ-TEMPLATES-010: Archive Skill Template
Entry Gate（verified + knowledge synced；`scale: quick` 增列 spec-impact 檢查與 diff 路徑模組推導）→ 6 階段流程：Scan → Summary → Archive → Feature Spec Sync → Product Spec Regeneration → Knowledge Sync Re-check。完成度依 kind 判讀——未完成 code task 警告列示，manual 未勾選提醒不阻擋。
- WHEN Entry Gate, THEN 要求 status=verified 且受影響模組（delta-spec ADDED/MODIFIED/REMOVED REQ 前綴）README 反映變更最終狀態；未同步 → FAIL 阻擋並導向 `/prospec-knowledge-update`
...
- WHEN Phase 4, THEN 複核 gate 狀態維持（無互動 prompt）；`archive.service` 的 non-fatal knowledge-update 為冪等 safety net（REQ-SERVICES-010 不變）
...
**Acceptance Scenarios:**
- WHEN 對指定 module 觸發 `/prospec-backfill-spec` 且來源有足夠行為線索 THEN 於 `.prospec/changes/[name]/backfill-draft.md` 產出 route-compatible 草稿（`**Feature:**`/`**Story:**` + US/AC 候選）
...
#### REQ-TEMPLATES-104: prospec-backfill-spec 回填萃取
獨立 skill `prospec-backfill-spec.hbs` 執行回填萃取：多源 triangulation 依 source→field 對照填欄（code+tests→behavior+AC、git body→*So that*、docs/README→role/value/目標用戶、ai-knowledge→僅 module routing），先枚舉行為再聚類全部成數個 US 並明列 deferred，產出 route-compatible 草稿至 `.prospec/changes/[name]/backfill-draft.md`。獨立 Lifecycle skill（hasReferences:false），不新增 reference 檔。
- WHEN 觸發 prospec-backfill-spec, THEN 依 source→field 對照產出 route-compatible 草稿（`**Feature:**`/`**Story:**` + US/AC）
...
#### REQ-TEMPLATES-108: prospec-backfill-spec 獨立 Lifecycle skill
新增獨立 Lifecycle skill `prospec-backfill-spec`（type=Lifecycle、hasReferences:false），承接 brownfield WHAT-layer 回填能力；triggers 含 backfill/補規格/回填規格/反向萃取/從程式碼產規格；skill 總數 14→15，列入 entry config，prospec-design 回歸純 Generate/Extract。
- WHEN 使用者以 backfill/brownfield 類語句觸發, THEN 喚起 `/prospec-backfill-spec`（無須 input=code 參數）
- WHEN 檢視 prospec-design, THEN 不再含 input=code 反向變體（純 Generate/Extract UI）
