...
- WHEN 歸檔完成 THEN 生成 summary.md（knowledge 同步由 Entry Gate 強制；service 層不自動觸發 knowledge-update/raw-scan）
...
`archive.service.execute()` 不再於歸檔後自動觸發 `executeKnowledgeUpdate`（→ `updateIndex`）或 `generateRawScan`——因無 `prospec archive` CLI 指令、且 auto knowledge-update 的 `updateIndex` 會清空 curated `index.md` 表。knowledge 同步由 `/prospec-archive` skill Entry Gate 強制、module README 由 verify S/A commit 折入；手動逐 phase 是唯一路徑。`ArchiveResult` 不含 `knowledgeUpdated`/`knowledgeWarnings`/`rawScanRefreshed`（`generateProductSpec`/`syncFeatureMap` 保留）。
- WHEN `execute()` 歸檔完成, THEN 不呼叫 `executeKnowledgeUpdate`、不呼叫 `generateRawScan`
- WHEN 檢視 `ArchiveResult`, THEN 不含 knowledgeUpdated/knowledgeWarnings/rawScanRefreshed 欄位
- WHEN 檢視 prospec-archive skill 模板, THEN 無「service 自動觸發 knowledge-update/raw-scan safety net」反向宣稱
...
skill-end 摘要折入 Exit Gate：**非 verify** 站把「比對整部 Constitution」收窄為該站**站點特定規則**（review→dependency/layering、learn→promotion-approval、new-story→INVEST、plan→dependency-direction、tasks→TDD），仍消費 BL-031 severity（MUST→FAIL/SHOULD→WARN/MAY→資訊性）並把 WARN/FAIL 記入 metadata `quality_log`（US-12 跨階段追溯不變）；verify 的 Exit Gate 維持整部 Constitution 全審（唯一全審站，REQ-TEMPLATES-133）。Exit advisory，不硬阻擋。
- WHEN 非 verify skill 結束, THEN skill-end 摘要含**站點特定** Constitution 結果（依 severity 分級），非整部全審
...
#### REQ-TESTS-035: Feature-Map 完整性與 no-clobber 測試
feature-map `mcp-server.modules` 完整性（真檔契約）+ `syncFeatureMap` no-clobber 不縮減 curated 集。
...
`tests/contract/skill-format.test.ts` section-scoped 斷言：verify fidelity 主軸/既有違規降 informational/測試分流/Entry Gate 例外 + provenance/1-5 N/A、archive 接受 + 模組推導 + Phase 2 skip + Phase 3.5 arm、review Entry Gate 例外、promote 輕量 scaffold（無 plan/tasks）、`SKILL_DEFINITIONS` count 16。全部 mutation-verified（PB-001）。
...
## US-25: Constitution 全審收斂到 verify 單站 [P1]

身為一名維護 SDD 流程成本的 prospec maintainer，
我想要 Constitution 完整分級稽核只在 `/prospec-verify` V3/5 一處執行、其餘站只檢查站點特定規則，
以便每變更 Constitution 檢查從 ≥7 次收斂到 1 次全審＋引用，砍掉重複稽核成本而工程紀律不減。

**Acceptance Scenarios:**
- WHEN new-story/plan/tasks/ff/implement 各站 Constitution 觸點執行，THEN 只檢查站點特定規則、不做通用「3+ principles」全掃
- WHEN 非 verify 的 Exit Gate 執行，THEN 對站點特定規則比對並記 `quality_log`（US-12 保留），不重評整部 Constitution
- WHEN verify V3/5 執行，THEN 為唯一的 Constitution 全分級稽核（every principle）
- WHEN 檢視 archive/design/backfill-spec/promote-backfill/knowledge-update，THEN 無載入後未消費的 Constitution `[STABLE]` 項；ff 無「NEVER skip Constitution check at any phase」；Entry Gate 的 constitution-exists 存在性檢查保留
- WHEN 統計單一 standard/full 變更，THEN Constitution 全審恰 1 次（verify），其餘站為站點特定引用

### Behavior Specifications

#### REQ-TEMPLATES-133: Constitution 全審收斂到 verify 單站
Constitution 完整分級稽核（every principle）只在 `/prospec-verify` V3/5 執行；規劃/執行各站只檢查站點特定規則（new-story→INVEST、plan→dependency-direction/layering、tasks→TDD 測試覆蓋、implement→TDD/commit、ff per-phase），不做通用「3+ relevant principles」全掃。移除 ff「NEVER skip Constitution check at any phase」與 archive/design/backfill-spec/promote-backfill/knowledge-update 中載入後未消費的 Constitution `[STABLE]` 項；Entry Gate constitution-exists 存在性檢查與 verify 全審維持。
- WHEN 非 verify skill grep，THEN 無「every principle / full audit / 3+ … principles」全審措辭；verify 維持全審
- WHEN 檢視 ff，THEN 無「NEVER skip Constitution check at any phase」
- WHEN 檢視上述 5 skill 的 Startup Loading，THEN 無 Constitution `[STABLE]` 載入；new-story/plan/ff 的 constitution-exists Entry Gate 檢查保留

#### REQ-TESTS-044: Constitution 收斂 contract 斷言
`skill-format.test.ts` section-scoped + mutation-verified 釘住收斂：verify 維持 full-audit；非 verify 站 negative assertion（無全審措辭）；`prospec-ff` 無「NEVER skip Constitution check at any phase」；指定 5 skill 無 orphaned Constitution `[STABLE]` 載入；startup-loading baseline 同步。
- WHEN contract runs，THEN 正向（verify full audit、站點特定措辭）＋負向（非 verify 無全審、ff 無 NEVER-skip、orphaned 清零）皆綠
- WHEN 移除/還原任一目標措辭，THEN 對應斷言轉紅
...
**Reason**: archive.service 的 auto knowledge-update 整段移除（無 CLI 驅動、`updateIndex` 會清空 curated index），「對 backfill 跳過」隨之 moot。knowledge 同步統由 archive skill Entry Gate 負責。

#### ~~REQ-SERVICES-033: Archive Auto Knowledge-Update 轉發 related_modules~~
**Removed**: 2026-07-05 | **Change**: remove-archive-auto-knowledge-update
**Reason**: auto knowledge-update 移除後，「轉發 related_modules 給 auto-update」不復存在。feature-prefix 模組推導改由 archive skill Entry Gate（`related_modules`/`**Feature:**`→feature-map）負責。
