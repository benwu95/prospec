...
- WHEN called in a git work tree, THEN it returns the union of `git diff --name-only HEAD` and untracked files over the same denylist scope as the change digest (generated `src/**` artifacts included; `.prospec`, `.claude`, `.agents`, `dist`, lockfiles excluded)
