...
`/prospec-knowledge-update`'s mechanical work collapses into a single `prospec knowledge update` call plus a review of its report, run as Phase 1; that report surfaces diff-attributed stamp-only modules — those a working-tree diff attributes but no REQ named (generated artifacts) — which need only a freshness stamp, not a README edit. Phase 2's source scan and description inference, Phase 2.5's format-drift consent dialogue, and Phase 3's README content authoring and verification stamp stay the skill's; Phase 3's 3e defines the affected modules to stamp as the delta-spec REQ-prefix modules ∪ diff-path attributed modules (generated included), and notes the project's knowledge-sync gate (if any) is re-run after the feature commit, before push.
...
- WHEN Phase 1 reviews the report, THEN it surfaces the diff-attributed (stamp-only) modules the command reports — modules a working-tree diff attributes but no REQ named — as needing only a freshness stamp, not a README edit
- WHEN 3e names the affected modules, THEN it defines them as REQ-prefix ∪ diff-path (generated included) and notes the project's knowledge-sync gate, if any, is re-run after the feature commit and before pushing, worded project-agnostically with no literal script, filename, or CI-provider name
- WHEN a negative scan runs over the rendered `prospec-verify` and `prospec-knowledge-update` skills, THEN neither contains a repo-internal token (`knowledge:check`, `check-knowledge-sync`, `bundled-templates`) — including the pre-existing `knowledge:check` literal previously at the end of §3e, which is generalized to the project's knowledge-sync gate

---

#### REQ-LIB-062: Working-tree changed-path attribution primitives
`changedPathsFromWorkTree(cwd)` lists the repo-relative paths a change has touched in the working tree, and `partitionDiffAttributedModules` maps them to modules and separates the ones no requirement named.
- WHEN called in a git work tree, THEN it returns changed tracked paths plus non-ignored untracked paths through lossless NUL-delimited Git capture and the same effective-input scope owner as snapshot-v2; generated source, documents and lockfiles remain included, while only `.prospec/` and Prospec-owned report constants are excluded
- WHEN the directory is not a git work tree, or any git capture fails, THEN it returns null (fail-closed), never an empty list read as "nothing changed"
- WHEN `partitionDiffAttributedModules(changed, moduleMap, acknowledged)` runs, THEN it attributes each path via `moduleAttributor`, returns the attributed modules as `diffAttributed`, and returns those absent from `acknowledged` (case-insensitive) as `stampOnly`
- WHEN a changed path is attributed to no module, THEN it contributes to neither set

---

#### REQ-SERVICES-097: knowledge update --change reports diff-attributed stamp-only modules
`prospec knowledge update --change` reports the modules a working-tree diff attributes but no delta-spec REQ named — typically generated artifacts — as stamp-only candidates, without touching their READMEs.
- WHEN `executeForChange` resolves a change's delta-spec, THEN it computes the change's working-tree diff-attributed modules and returns those not already acknowledged by a REQ prefix as `stampOnly`
- WHEN a module is already acknowledged (created / updated / readme-pending / deprecated), THEN it is not repeated in `stampOnly`
- WHEN the working tree is unavailable (not a git repo or a git capture fails), THEN `stampOnly` is empty and the REQ-prefix behavior is unchanged
- WHEN manual `--module` mode runs, THEN no diff attribution is performed and `stampOnly` is empty — stamp-only applies to change mode only (the field is always a total `string[]`, never absent)

---

#### REQ-CLI-042: knowledge-update output prints a stamp-only section
The knowledge-update output prints diff-attributed stamp-only modules as a section distinct from README-pending.
- WHEN the result carries `stampOnly` modules, THEN the formatter prints a `stamp-only (diff-attributed, e.g. generated files)` section listing them, separate from `README content pending`
- WHEN `stampOnly` is empty, THEN no such section is printed
- WHEN a module name is rendered, THEN it passes through `sanitizeTerminal`
...
#### REQ-TEMPLATES-207: Verify Spec-Knowledge Staleness Detection
- WHEN delta-spec MODIFIED but module README not updated, THEN emit an informational note pointing to final Knowledge/count sync before final review/tests/verify, with archive as the backstop; the note does not count toward the grade, agrees with the same-named sdd-workflow requirement, and never preserves old evidence after effective inputs change
