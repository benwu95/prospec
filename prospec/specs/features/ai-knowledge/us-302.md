...
- WHEN a Module README is generated or migrated to the current grammar, THEN it carries `<!-- prospec:module-readme-format 2026-09-01 -->` after its summary; the date identifies the compatible format release rather than the document modification time, and registered extension sections plus freeform user content are preserved in the user block
- WHEN a format change only clarifies prose or adds a registered optional extension, THEN it retains the existing format date; an incompatible change to title/summary placement, marker semantics, or Core section grammar requires a new date
...
- WHEN `_module-readme-conventions.md` declares Project Section Extensions in its preserved registry, THEN it defines the fixed Core grammar and all registered extension fields/instance syntax; `.prospec.yaml` does not define Module README section structure
