...
- WHEN referenced, THEN it documents dual-axis circuit breaker stopping rules
...
#### REQ-TYPES-089: Review Finding Origin Round and Spend Contracts
`ReviewFindingSchema` and `ReviewRow` gain `origin_round` (positive integer indicating the review round in which the finding was first detected). `CircuitBreakerConfigSchema` gains `maxFixInducedRatio` and `maxSpend`. `EscalationReportSchema` gains `fix_induced_threshold_exceeded` and `spend_budget_exceeded` escalation types.
- `ReviewFindingSchema` accepts optional `origin_round` (positive integer).
- `CircuitBreakerConfigSchema` validates `maxFixInducedRatio` (0.0–1.0, default 0.5) and optional `maxSpend` (non-negative integer).
- `EscalationReportSchema` supports new escalation types: `fix_induced_threshold_exceeded` and `spend_budget_exceeded`.

---

#### REQ-LIB-061: Dual-Axis Circuit Breaker and Fix-Induced Ratio Calculation
`lib/oscillation-breaker.ts` is extended to evaluate dual-axis circuit breaker conditions: tracking fix-induced ratio across rounds and monitoring cumulative token spend against declared budgets, in addition to oscillation flips and round caps.
- Computes `fixInducedRatio` for rounds > 1 based on findings with `origin_round > 1` divided by total round findings.
...
- Unit tests verify `review-merge` origin tracking, `OscillationBreaker` dual-axis triggers, and `review-merge-output` formatting.
