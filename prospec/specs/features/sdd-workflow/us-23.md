...
`skill-format.test.ts` section-scoped + mutation-verified pins: review records provenance each round, the verify Entry Gate blocking wording (negative: no "Absence does NOT block verify"), the grep-hit of PB-001/003/006/007 in the corresponding template including their current strengthened clauses (PB-003 enforcement-face + no-mechanism-face, PB-007 remediation), and — reading the real `_playbook.md` — that every `Inlined into gate`/`Mechanized` annotation's `Landing:` anchor names an existing file containing the declared marker.
