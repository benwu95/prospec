...
- WHEN the original acceptance baseline is unavailable or late-captured THEN retain each actual code-fidelity judgment, disclose the original-baseline gap and prevent S without masking a FAIL
...
- WHEN backfill 2/5 runs, THEN the verify-backfill reference applies the shared prepared-context and per-REQ items contract, preserving each actual code-fidelity PASS/WARN/FAIL
- WHEN the original pre-implementation baseline is unavailable or the current revision is late-capture, THEN disclose that gap as not-adjudicated, make S unreachable with the shared dimension-level warning, preserve any actual FAIL, and permit A when the existing grade budget and other gates allow it
...
- WHEN new review provenance is written, THEN it additionally carries the recognized `fingerprint_version` and `scope` plus optional Git `head` trace; the snapshot identity excludes that trace
- WHEN old provenance omits version/scope or carries an unknown version, THEN it remains readable without silent promotion to current evidence; a normal completed review establishes the new version once, and later content-equivalent commits preserve it

#### REQ-TEMPLATES-130: prospec-review records provenance each round
`review merge` writes a `skill: prospec-review` `quality_log` counts entry on each round (including a review-clean round, 0 critical / 0 major, whose counts are zeros), and `prospec check --record-review` records the baseline after the loop converges; the skill runs `change log --skill prospec-review` per round only to close the round and to audit its self-reported counts, its §Provenance and Clean-review prose reduced to a one-line pointer to the CLI behavior.
- WHEN a round completes, THEN the CLI-written quality_log carries a machine-parseable prospec-review counts entry for that round and, at loop convergence, the baseline is stamped
- WHEN the round is review-clean, THEN `review merge` records the zero counts and writes the artifact-language clean sentence, so the round is distinguishable from one that never ran without any hand-appended prose
- WHEN the CLI is unavailable, THEN the skill states the fallback explicitly and never silently skips
- WHEN the skill template is read, THEN its §Provenance counts explanation and Clean-review callout are a one-line pointer ("the CLI records this round's counts and clean sentence at merge time; `change log` only closes the round and audits self-reported counts"), and the Loop's six-step order and reference list are unchanged

#### REQ-TEMPLATES-131: prospec-verify Entry Gate blocks absent/stale review
The review-provenance gate is enforced by `prospec verify record`, which refuses to record a non-backfill verify verdict when the live assessment's `review-provenance` check, adjudicated for the target change through `adjudicateChangeCheck`, is not `pass` (absent, stale or unprovable) and prints the remedy; a proven backfill stays exempt because Gate A is guarded by the proven-backfill not-applicable policy (the evaluator emits no finding for it, so the per-target adjudication yields `pass`). The machine dimensions `task-completion` and `tests` are likewise adjudicated for the target (an `unprovable` verdict records `not-adjudicated` with its reason), while `knowledge` keeps the repository-wide check. The `prospec-verify` Entry Gate defers to that refusal in a single line rather than reading `prospec check` itself, and the shared CLI probe STOPs the skill when the engine is absent, so there is no hand-run fallback.
- WHEN non-backfill and review-provenance FAIL, THEN `verify record` refuses to record; WHEN PASS, THEN recording proceeds normally
- WHEN backfill, THEN exempt
- WHEN required live evidence is unprovable, legacy or changed before the write, THEN the CLI refuses with its cause-specific remedy; neither an old report PASS nor a clean Git tree substitutes for a completed current review
- WHEN a sibling change has no review recorded but the target's review is current, THEN the target's verdict records; WHEN the target itself is not enumerated by the check, THEN recording is refused as unprovable
...
#### REQ-SERVICES-100: verify record refuses on failed review-provenance
`prospec verify record` reads the shared live assessment's `review-provenance` check, validates its observation receipt before any write, and refuses to record a non-backfill verify verdict when that check is FAIL (review absent or stale), printing the remedy; a justified applicability `skipped` check (a proven backfill, or no review by scale) or a current `pass` check records normally; an unprovable required assessment does not authorize a write. The refusal precedes any write, so metadata stays byte-identical.
- WHEN the change is non-backfill and `review-provenance` is FAIL, THEN `verify record` exits non-zero and prints the remedy (run `/prospec-review`, then `prospec check --record-review`)
- WHEN `review-provenance` is an applicable current `pass` or a justified applicability `skipped` and the observation receipt remains valid, THEN recording proceeds unchanged; unavailable required evidence is not a current PASS

---

#### REQ-SERVICES-101: verify record floors a judgment dimension against its machine counterpart
`prospec verify record` refuses, before any write, when a judgment dimension is graded more leniently than the drift report's machine counterpart for the same subject. Only `constitution` has a clean counterpart today (the `constitution-severity` check): a report `fail` requires the judgment to be `FAIL`, a report `warn` requires `WARN` or `FAIL`. A `pass`, `skipped`, or absent check sets no floor, and the reverse — a machine `pass` with a stricter judgment — is allowed, because a judgment dimension detects violations the machine cannot.
- WHEN `constitution` is declared PASS/`not-applicable`/`not-adjudicated` while `constitution-severity` is FAIL or WARN, THEN `verify record` refuses and names the contradiction
- WHEN the `constitution-severity` check is `pass`/`skipped`/absent, THEN no floor applies; a judgment stricter than the machine finding is always allowed
