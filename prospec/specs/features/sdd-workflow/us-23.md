...
The review-provenance gate is enforced by `prospec verify record`, which refuses to record a non-backfill verify verdict when the live assessment's `review-provenance` check is FAIL (absent or stale) and prints the remedy; `scale: backfill` stays exempt because a proven backfill's check is `skipped`. The `prospec-verify` Entry Gate defers to that refusal in a single line rather than reading `prospec check` itself, and the shared CLI probe STOPs the skill when the engine is absent, so there is no hand-run fallback.
