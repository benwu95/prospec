...
`ChangeMetadataSchema` adds an optional `review_provenance {digest, date, graded_by?, executor?}` (a code-computed review baseline, compared by the `review-provenance` check; `graded_by` is the reviewer's self-declaration of the grading context, `fresh-subagent`|`in-session`; `executor` is the reviewer's self-declared executor label, a non-empty string validated against the project's declared vocabulary at the write path), alongside `quality_log` (REQ-TYPES-022); omitting it — or omitting only `graded_by` or `executor` — is still backward-compatible (metadata is read losslessly, a type contract).
...
- WHEN `review_provenance` carries `executor`, THEN it is a non-empty string; omitting it still validates
...
`prospec verify record` reads the already-parsed drift report's `review-provenance` check and refuses to record a non-backfill verify verdict when that check is FAIL (review absent or stale), printing the remedy; a `skipped` check (a proven backfill, or no review by scale) or a `pass` check records normally. The refusal precedes any write, so metadata stays byte-identical.
...
- WHEN `review-provenance` is `skipped` or `pass`, THEN recording proceeds unchanged
