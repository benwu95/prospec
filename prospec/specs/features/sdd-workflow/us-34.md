...
The `spec show` service returns the slices a caller selected from one feature spec, routing feature resolution, the contained read, selector expansion and slice selection through the shared `lib/spec-read` entry and applying only its own no-selector policy — so containment, the `_archived*` exclusion, the unsafe-name guard and the selection rule live in one place the MCP tool shares, and the service parses no spec of its own.
...
- WHEN the shared entry reports no usable selector, THEN this service applies its own policy: it prints the whole spec when no flag was given at all, and refuses when a flag was given but carried no usable id
...
- WHEN the two narrow-read surfaces are inspected, THEN a contract assertion pins that both the CLI `spec show` service and the MCP `get_spec_requirements` tool route feature resolution, contained read, selector expansion and slice selection through the one shared `lib/spec-read` entry, and that neither calls `indexSpec` or `selectSpecSlices` itself

#### REQ-LIB-055: One shared entry resolves, reads and selects a feature spec for both surfaces
`lib/spec-read.ts` is the one entry below the service layer that turns a feature name plus optional selectors into the source a narrow-read surface serves: it resolves the feature, reads it through the contained reader, expands the selectors and selects the slices, so the CLI command and the MCP tool share resolution and parsing rather than one copy each.
- WHEN `readSpecSlices(featuresDir, feature, selectors)` is called, THEN it returns one of three results — `not-found` carrying the `isSafeResourceName`-filtered list of specs that do exist, `no-selector` carrying the read content and the raw flag count, or `sliced` carrying the `selectSpecSlices` selection and the expanded selectors
- WHEN selectors arrive comma-separated, repeated as flags, or both, THEN the entry expands them to one selector set, so no surface expands its own and the two input shapes cannot disagree
- WHEN a surface needs the whole spec text, THEN `assembleWholeSpec(content)` produces it — the main file followed by each slice body — so the CLI whole-spec fallback and the MCP `spec://feature/{name}` resource cannot disagree on what the whole spec is
- WHEN the entry is assembled, THEN it composes `knowledge-reader`, `spec-headings` and `spec-slices` with no lib→lib cycle, and the pure selection (REQ-LIB-046) is left unchanged
