...
#### REQ-SERVICES-096: Refuse a MODIFIED or REMOVED REQ routed to a feature that does not host it
`archive.service`'s spec sync refuses a MODIFIED or REMOVED REQ whose `**Feature:**` header names a feature that does not host that REQ id while the REQ lives in another feature, rather than appending it as a stale duplicate section in the wrong feature. The declared feature is resolved through the single shared routing classifier the `delta-spec-landing-fidelity` check also consults, so the refusal and the check verdict cannot diverge. The refusal is a blocking `refusedRequirements` outcome folded into the loss verdict: the feature spec is left byte-identical and the run exits non-zero, and dry run reports it identically. A REQ that lives in no feature yet is not a mis-route — a create-and-deprecate REMOVED lands its deprecation record as before. A legitimate cross-feature move is expressed by an explicit declaration, never by a mis-pointing header.
- WHEN a MODIFIED or REMOVED route's declared feature does not host its REQ id while the REQ lives in another feature, THEN the REQ is refused, the feature spec is left byte-identical, and the run exits non-zero
- WHEN such a REQ is refused, THEN the refusal names the feature that actually hosts the REQ id
- WHEN the declared feature does host the REQ id, THEN the REQ merges in place exactly as before
- WHEN a REQ id is defined in no feature yet, THEN it is not refused and lands as before
- WHEN running with dry run, THEN the refusal is reported identically and nothing is written
