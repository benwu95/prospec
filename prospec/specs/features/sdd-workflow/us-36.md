...
- WHEN a delegated spec-compliance grader returns its receipt THEN include context_id, per-REQ evidence and applicable scenario findings; inspect the real non-empty file against the executable schema before recording
- WHEN requirement rows or coverage gaps exist without grader prose THEN verify.md still records them with the context identity; metadata keeps only verdicts and lightweight identities
...
The `review merge` service performs every pre-existing input refusal before the first byte reaches disk — the relayed-field ceilings and their single-line rule, the required `repro` on a critical, the required `id` behind evidence, and a marker in ANY field that reaches a raw evidence line (the prose and the `id` anchoring it alike) — and only then permits persistence. The sole additional refusal-time write is a bounded test-failure metrics update; an accepted round still writes review.md once.
- WHEN a round is refused for payload validation, evidence grammar, round sequencing, malformed metrics or unstable observations before the first write, THEN this invocation writes no files and an absent review.md is not created.
...
- WHEN only the test gate refuses and a newly observed failure changes its metrics, THEN atomically replace only the test-related metrics attributes, preserving all bytes outside the metrics comment and all existing non-test metric values; never call the full findings/evidence renderer on this refusal path.
- WHEN test metrics are the only persisted state and review.md was absent, THEN the documented metrics-only file creation is permitted; findings, evidence, round and spend are not created or advanced.
- WHEN a pre-write test refusal produces no new test metric, THEN this invocation writes no files.
- WHEN the initial test gate refuses, THEN metadata remains byte-identical, and an I/O failure is reported honestly instead of claiming the metrics were recorded.
- WHEN an initially allowed exemption has already persisted its truthful WARN and subsequent revalidation refuses, THEN that warning may remain as the sole metadata exception; this invocation does not write review.md or a completed-round log, does not roll back concurrent edits, and reports the partial warning-only outcome.

---


#### REQ-SERVICES-087: `verify record` appends judgment evidence to `verify.md`
The `verify record` service accepts the judgment dimensions' evidence as optional structured input and appends it to `verify.md` under a heading carrying the run's date and grade, while the existing grade table receives the requirement-assessment floor and the metadata records verdict plus lightweight context/baseline/coverage identity; the artifact records why.
- WHEN evidence is supplied, THEN the `quality_log` entry carries verdicts and lightweight context/baseline/coverage identity: a dimension's evidence lands in `verify.md`, never in `metadata.yaml`
...
- WHEN neither judgment prose nor per-REQ results, coverage gaps or findings exist, THEN no empty verify.md is written; otherwise the report records the context, item rows and findings even when the caller supplied no prose
...
- WHEN evidence validation or a prewrite recheck refuses, THEN all target artifacts remain byte-identical; if the metadata write succeeds but the later evidence write fails, THEN report that partial outcome honestly without claiming rollback

---


#### REQ-CLI-037: `review merge` reports the round as a bounded digest
The `review merge` command's output is the orchestrating context's whole intake for a review round: it names the artifact it wrote, how many evidence blocks that artifact now holds, the round's counts, and — for each critical — the finding's id, location, lens, summary and the command that reproduces it. When the round escaped at least one table cell, one further line states the escaped-cell count and the rule.
...
- WHEN the round escaped no cell, THEN the output is byte-identical to the digest before this change; WHEN it escaped at least one, THEN exactly one extra line reports the count and the `|` / newline rule
...
- WHEN only the repeatable flag is given, THEN the smaller grammar remains available, but applicable missing REQ/context evidence is unadjudicated and the resulting coverage report is written
- WHEN any judgment prose, item rows, coverage gaps or scenario findings are recorded, THEN the output names verify.md alongside the grade; only a run with none of these writes and names no report
...
- WHEN the file form carries items, context_id or scenario_findings, THEN only delta-spec-compliance may own them; output relays bounded counts, identifiers and report paths rather than evidence prose
...
- WHEN the shared contract is read, THEN it documents spec-compliance items, context_id and scenario_findings by reference to the executable schema and explicitly permits checkable document/architecture evidence without fabricated executable repro
- WHEN the reference renders, THEN it contains three fixed examples: an acceptable FAIL with checkable evidence, an unacceptable PASS without evidence, and a false-positive critical dismissed after its alleged reproduction fails to demonstrate the claim
...
All subagent-delegating stations and references (`prospec-review`, `prospec-verify`, `prospec-plan`, `prospec-tasks`, `prospec-ff`, `delegated-evidence-format.md`, `candidate-evaluation.md`, `plan-verifier-rubric.md`, `tasks-verifier-rubric.md`) MUST enforce the Physical Receipt Verification Protocol. Review and verify payloads MUST conform to the executable `ReviewFindingsInputSchema` and `JudgmentDimensionsInputSchema` authorities, which `delegated-evidence-format.md` documents without redefining; planning reports use the executable schema their station owns (`PlanVerifierReportSchema` / `TasksVerifierReportSchema`, `CandidatePayloadSchema` / `DecisionPayloadSchema`), which the relevant reference documents without redefining.
