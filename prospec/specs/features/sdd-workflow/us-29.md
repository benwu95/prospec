...
#### REQ-LIB-062: Origin Round and Lens Tracking in Review Merge
`lib/review-merge.ts` tracks `origin_round` on findings rows, preserving initial origin round when carrying forward or updating existing findings, and renders `Origin` in the review table.
- Newly discovered findings in round $N$ are assigned `origin_round: N` if not explicitly specified.
...
#### REQ-SERVICES-097: Review Merge Service Dual-Axis Bookkeeping
...
- Accepts optional `round`, `spend`, `budget`, `maxFixInducedRatio`, `maxRounds`, and `maxFlips` options.
- Evaluates circuit breaker via `OscillationBreaker` and returns `circuitBreaker` state in result.
- Automatically derives round number from history if not explicitly provided.

---

#### REQ-CLI-042: Review Merge CLI Flags and Dual-Axis Escalation Output
`prospec review merge` CLI command accepts `--round`, `--spend`, `--budget`, and `--max-fix-induced-ratio` flags, and `formatReviewMergeOutput` displays round metrics (fix-induced ratio, spend) and renders escalation warnings when circuit breakers trip.
- CLI registers `--round`, `--spend`, `--budget`, and `--max-fix-induced-ratio` options with type validation.
