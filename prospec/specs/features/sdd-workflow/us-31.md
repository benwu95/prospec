...
- WHEN `pnpm counts` runs, THEN the root READMEs and both website language sources receive total, passing, and skipped test counts from one derived report
- WHEN any declared website count is stale, missing, or changes shape, THEN `pnpm counts:check` or its section-scoped target contract fails instead of passing vacuously
...
#### REQ-TESTS-098: knowledge:check treats an empty commit range as a distinct skip
The `knowledge:check` gate treats an empty commit range as a distinct skip, not an all-confirmed pass.
- WHEN the commit range is empty (HEAD is the merge-base, no changed paths), THEN the gate prints a skip message telling the developer to commit first and re-run, and does NOT print the `all confirmed` form
- WHEN the range is non-empty and every source-touched module is confirmed, THEN the gate prints the all-confirmed line unchanged
- WHEN the range is non-empty and a source-touched module is unconfirmed, THEN the gate names it and exits non-zero, unchanged
- WHEN the outcome is classified, THEN a pure function decides `empty-range` / `violation` / `confirmed`, so all three are unit-tested without invoking git
...
- WHEN `scale: quick` and no plan.md exists and the declared acceptance contract has a frozen revision (or legacy metadata declares no contract), THEN `change tasks` scaffolds tasks.md and advances `story → tasks` without producing plan.md or delta-spec.md
...
- WHEN the scale is `standard`, `full`, or absent, THEN both stations keep their existing prerequisites, including missing-plan.md refusal at tasks; plan additionally refuses a declared but pending acceptance baseline before writing
...
- WHEN a declared acceptance contract is still pending, THEN the plan or quick-tasks entrance names the story-freeze remedy without auto-capturing; legacy absence remains admissible with explicit limitation
...
- WHEN a new quick path runs story → scale quick → author substantive scenarios → freeze → tasks, THEN the integration test asserts tasks.md exists, plan.md and delta-spec.md do not, and status is tasks
- WHEN new-contract story enters plan or quick tasks before freezing, THEN tests assert actionable refusal and byte-identical artifacts; legacy metadata without the acceptance contract still exercises the prior admissible path
