...
- WHEN any code sink in src/ reads or writes metadata issue, THEN issue-ref-sink contract test mechanically verifies that it routes through this helper
...
- WHEN the archive summary is generated for a change carrying `issue`, THEN the Change Overview block gains one `- **Issue**: <ref>` line; without it, the block carries no Issue line
...
#### REQ-TESTS-105: Issue-Ref Sink Contract Test
`tests/contract/issue-ref-sink.contract.test.ts` asserts that all sinks reading or writing the change metadata `issue` property route through `normalizeIssueRef`.
- WHEN the contract test runs, THEN every identified read/write sink for `metadata.issue` is verified to call `normalizeIssueRef`
- WHEN any sink bypasses `normalizeIssueRef`, THEN the contract test turns red (mutation-verified)
