...
- WHEN 一次 verify 對同一類問題反覆 FAIL THEN 記入個人教訓清單並標來源 change 與出現次數
- WHEN archive 偵測到與歷次變更重複的錯誤模式 THEN 匯入個人教訓清單
...
`prospec-learn` Collect + Score：掃 archived changes 的 quality_log/review.md + 既有 ledger，依**確定性 key** 萃取/配對、增量更新 frequency/impact_modules/scope/source（落 `.prospec/lessons.md`，gitignored）；Score 套**明文數值規則**標「建議升級」附計分明細。
...
`references/promotion-format.md`：明文晉升規則（預設 freq≥3 / impact_modules≥2，可由 `.prospec.yaml` 覆寫）+ ledger / playbook entry / 核可記錄 / TTL 四種結構。規則明文化＝可重現/可審計的根據（reproducibility 條件於穩定 ledger key）。
- WHEN referenced, THEN 含明文數值門檻 + `.prospec.yaml` 可設定 + 四種結構定義
...
Govern：共享規則帶 TTL 與來源；過期/衝突 → 待 review 清單，淘汰於版控留原因。建 `_playbook.md`（版控）並登錄 `_index.md` Conventions；plan/implement Startup 載入**相關** playbook（漸進揭露）；archive Phase 4.5 加「建議 `/prospec-learn`」。
