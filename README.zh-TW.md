...
[![測試](https://img.shields.io/badge/測試-1748%20通過-success?style=flat-square)](tests/)
...
- **`prospec upgrade`（CLI）** 在 `.prospec.yaml` `version` 記錄當前 prospec 版本（就地合併、保留你的註解與格式）、重跑 `agent sync` 讓各 agent 設定與 Skills 對齊最新模板，並印出 migration report（版本差異；接著在專案從未選擇語言時（例如由 pre-feature CLI 建立的專案）提示可設定 `artifact_language`，否則列出任何缺母語觸發詞的新 skill）。在互動式終端機中,它會像 `prospec init` 一樣逐一提示你填寫這些 nudge;piped/CI 執行——以及 `/prospec-upgrade` skill——會帶 `--no-interactive`,只取得報告。它**不寫任何 doc**——絕不碰 `CONSTITUTION.md`、`_conventions.md`、`_index.md`、canonical convention docs，或任何 module README。
...
| `prospec upgrade [--cwd <dir>]` | prospec 版本升級後：在 `.prospec.yaml` 記錄 prospec `version`（就地合併、保留註解與格式）、重跑 `agent sync`、印出 migration report，接著交棒給 `/prospec-upgrade`。不寫任何 doc —— init 建立檔／canonical doc 的格式更新由需同意的 skill 處理 |
...
**測試覆蓋率**：1748 個測試橫跨 4 大類：
- Unit tests（types + lib + services + cli）：1137 tests
- Contract tests（CLI 輸出 + Skill 格式）：554 tests
