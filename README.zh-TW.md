...
[![測試](https://img.shields.io/badge/測試-930%20通過-success?style=flat-square)](tests/)
...
- **Node.js** >= 22.13.0
- **AI CLI**（至少一個）：[Claude Code](https://docs.anthropic.com/claude/docs/claude-code)（推薦）、[Antigravity CLI](https://antigravity.google/)、[GitHub Copilot CLI](https://docs.github.com/copilot/github-copilot-in-the-cli) 或 [Codex CLI](https://developers.openai.com/codex/cli)
...
```bash
/prospec-ff add-my-feature    # 一次生成 story → plan → tasks
...
<details>
<summary>MCP Server —— 唯讀的專案真相層（任何 MCP agent）</summary>
...
**測試覆蓋率**：930 個測試橫跨 4 大類：
- Unit tests（types + lib + services + cli）：435 tests
- Contract tests（CLI 輸出 + Skill 格式）：445 tests
